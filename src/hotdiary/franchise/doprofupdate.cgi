...
$calinvite = goodwebstr $input{'calinvite'};
...
system "cat $ENV{HDHOME}/content.html";
system "java COM.hotdiary.franchise.FetchCal \"/cgi-bin/proxy/execproxyprofupdate.cgi?login=$login&biscuit=$biscuit&email=$email&calinvite=$calinvite&acceptit=$acceptit&fname=$fname&zone=$zone&informme=$informme&calpublish=$calpublish&checkid=$checkid&url=$url&pagertype=$pagertype&busp=$busp&cellp=$cellp&fax=$fax&pager=$pager&phone=$phone&country=$country&zipcode=$zipcode&state=$state&city=$city&street=$street&lname=$lname&rpassword=$rpassword&password=$password&rh=$rh&HDLIC=$HDLIC&vdomain=$vdomain&hs=$hs&jp=$jp\" \"$firewall\" \"$proxyPort\" \"$proxyHost\" \"$proxySet\" \"$ip\"";
