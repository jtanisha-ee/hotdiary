...
public class vcardio {
     ...
     String s, cgistr, jstr, statusprog, login; 
     int bytesRead, rec = 0;
     ...
     */
    public void parseVcard(String pprog, String login) {

        byte[] vcardrec = new byte[(int)size];
        int k, endindex, j = 0; 
	InitializeFields(); 
    ...
    public void vcard_read(String infile, String pprog, String userid, 
		String msgprog) {
       ...
       System.out.println("length of file = " + size1);
       size = (int)size1;
       inb = new byte[(int)size + 1];
       bytesRead = 0;
	 ...
	 inb[i] = 0;
       ...
       if ((checkValidityOfFile()) == 1) {
          parseVcard(pprog, login);
          msg = "Found+" + numrec +  "records." + "File+has+been+imported+successfully.<BR><DL></DL>";
	  msg = getCGIString(msg);
       }
 
       sendStatusMsg(statusprog, login);
    }
    ...
    void checkFN(int k ) {
	  ...
	  if (tlen == -1) {
	     msg = "FN is missing  :  in" + rec + "record"; 
	     msg = getCGIString(msg);
	     sendStatusMsg(statusprog, login);
          }
   ...
   void getTelDigits(int k, String telType, int telTypeLen, int pager, int work, int home, int cell, int fax) {
	  ...
	  // String upcaseString = telstring.toUpperCase();
          ...
          pager = telstring.indexOf(vcard.pager, 0);
          work = telstring.indexOf(vcard.work, 0);
          home = telstring.indexOf(vcard.home, 0);
          cell = telstring.indexOf(vcard.cell, 0);     
          fax = telstring.indexOf(vcard.fax, 0);
  ...
  void sendVcard(String pprog, String login) {

       // System.out.println("sendVcard \n");

       if (pprog != null) {
            try {
                Process p = Runtime.getRuntime().exec(pprog + " " + 
                  "login=" + login + " " + 
	          "fname=" + vcardin[rec].fn + " " +  
                  // "ver=" + vcardin[rec].ver + " " + 
                  "street=" + vcardin[rec].street + " " +
                  "city=" + vcardin[rec].city + " "  + 
                  "state=" + vcardin[rec].state + " " + 
                  "zipcode=" + vcardin[rec].zipcode + " " +
                  "country=" + vcardin[rec].country + " " +
                  "phone=" + vcardin[rec].home + " " + 
                  "pager=" + vcardin[rec].pager + " " +
                  "cellp=" + vcardin[rec].cell + " " + 
                  "busp=" + vcardin[rec].work + " " + 
                  "email=" + vcardin[rec].email + " " +
                  "url=" + vcardin[rec].url + " " + 
                  // "bday=" + vcardin[rec].bday + " " +
                  "organization=" + vcardin[rec].org + " " + 
                  "fax=" + vcardin[rec].fax + " " + 
                  "uid=" + vcardin[rec].uid + " ");
  ...
  void sendStatusMsg(String statusprog, String login) {

     if (statusprog != null) {
        try {
           Process p = Runtime.getRuntime().exec(statusprog + " " +
	       "msg=" + msg + " " + "login=" + login);
               InputStream is = p.getInputStream();
               byte b[] = new byte[256];
               int len;
  ...
  int checkValidityOfFile() {
      ...
      if (s.indexOf("BEGIN:VCARD", 0) < 0) {
	 return 0;
      }

      if (s.indexOf("END:VCARD", 0) < 0) {
	 return 0;
      }

      int begincnt = cntVcardsBegin();
      int endcnt = cntVcardsEnd();
      if (begincnt != endcnt) {
         msg = "This file has mismatched BEGIN and END";
	 msg = getCGIString(msg);
         return 0;
      }
      if (begincnt <= 0) { 
         msg = "This file is missing BEGINs and is not a valid VCARD format";
	 msg = getCGIString(msg);
         return 0;
      }
      if (endcnt <= 0) { 
