...
sub createCalendar {
   my($dy, $mo, $yr, $dp, $login, $biscuit, $vw, $f, $a, $h, $m, $tz, $en, $sc, $jvw, $g, $calp, $rh, $logo, $title, $banner, $vdomain, $hs, $jp, $os, $pw) = @_;

   hddebug "Enter createCalendar: jp = $jp, vdomain = $vdomain, g = $g, rh = $rh";
   if ($login eq "test") {
      status "If you are a competitor, please logoff the system immediately. All your transactions are being logged!";
      exit;
   }
...
# test generic functionality
   #if ($login eq "manoj") {
      #$showbiz = adjusturl "<a href=\"/cgi-bin/$rh/execdogeneric.cgi?pnum=3&p0=execfeature.cgi&p1=fcomp&p2=biscuit&fcomp=manoj&biscuit=$biscuit&re3=HTTP_COOKIE&le3=HTTP_COOKIE&re4=HTTP_COOKIE&le4=HTTP_COOKIE&enum=5&re0=CGISUBDIR&le0=rh&re1=HTTPSUBDIR&le1=hs&re2=SERVER_NAME&le2=vdomain\">Feature</a><BR><BR>";
...
# Init the base CGI url
   ...
   if ($biscuit eq "") {
       #if ($login ne "mjoshi") {
       #   $cgiscript = adjusturl "http://$vdomain/cgi-bin/$rh/execcalclient.cgi?sc=$sc&l=$login&g=$g&jp=$jp&pw=$pw";
       #} else {
          $cgiscript = "http://$vdomain/cgi-bin/$rh/execcalclient.cgi?sc=$sc&l=$login&g=$g&jp=$jp&pw=$pw";
       #}
   } else {
       #if ($login ne "mjoshi") {
       #   $cgiscript = adjusturl "http://$vdomain/cgi-bin/$rh/execcalclient.cgi?biscuit=$biscuit&g=$g&jp=$jp";
       #} else {
          $cgiscript = "http://$vdomain/cgi-bin/$rh/execcalclient.cgi?biscuit=$biscuit&g=$g&jp=$jp";
       #}
   }
...
# Init today ref
   ...
   if ($g eq "") {
      if (exists($portaltab{$login})) {
         $pubcolor = $portaltab{$login}{calcolor};
         $photo = adjusturl $portaltab{$login}{photo};
         $pubtitle = adjusturl $portaltab{$login}{title};
         $publogo = adjusturl $portaltab{$login}{logo};
         $tbanner = adjusturl $portaltab{$login}{tbanner};
         $bbanner = adjusturl $portaltab{$login}{bbanner};
         $but1 = adjusturl $portaltab{$login}{but1};
         $but2 = adjusturl $portaltab{$login}{but2};
         $but3 = adjusturl $portaltab{$login}{but3};
         $but4 = adjusturl $portaltab{$login}{but4};
         $audio = adjusturl $portaltab{$login}{audio};

         ## for publogo and pubtitle we want to use the same placeholders
         ## as label and logo in the hdcalviewtop.html
         if ($pubcolor eq "") {
	    $pubcolor = "ffffff";
         } 
	 if ($calcolor eq "") {
	    $calcolor = "ffffff";
         ...
         $prml = strapp $prml, "pubcolor=$pubcolor"; 
         $prml = strapp $prml, "calcolor=$calcolor"; 
         $prml = strapp $prml, "tbanner=$tbanner"; 
         $prml = strapp $prml, "bbanner=$bbanner"; 
         $prml = strapp $prml, "but1=$but1"; 
         $prml = strapp $prml, "but2=$but2"; 
         $prml = strapp $prml, "but3=$but3"; 
         $prml = strapp $prml, "but4=$but4"; 
         $prml = strapp $prml, "audio=$audio"; 

         ## photo will go here
         $prml = strapp $prml, "calphoto=";
         if ($photo ne "") {
            $prml = strapp $prml, "calphoto=$photo";
         }
	} else {
	  $pubtitle = "";
	  $publogo = "";
          $prml = strapp $prml, "pubcolor=ffffff"; 
          $prml = strapp $prml, "calcolor=ffffff"; 
          $prml = strapp $prml, "calphoto="; 
...
# Init the page header
   ...
   if ($g ne "") {
      ...
      if ($sc eq "p") {
         ...
         ## we want to use the same placeholders as before
	 ## this applies only to personal calendars
	 if ($g eq "" ) {
            if ($pubtitle ne "") {
               $label = $pubtitle;
	    }
         ...
         if ($calp  eq "CHECKED") {
            if ($hs eq "") {
               ...
               $label = adjusturl "$title<BR></b></FONT><FONT FACE=\"Verdana\" SIZE=\"2\">My Website (<a href=\"http://$vdomain/members/$login\">http://$vdomain/members/$login</a>)</FONT>";
            } else {
               #$label = adjusturl "$title<BR></b></FONT><FONT FACE=\"Verdana\" SIZE=\"2\">My Website (http://$vdomain/$hs/members/$alphaindex/$login)</FONT>";
               $label = adjusturl "$title<BR></b></FONT><FONT FACE=\"Verdana\" SIZE=\"2\">My Website (a href=\"http://$vdomain/$hs/members/$login\")http://$vdomain/members/$login</a></FONT>";
            }
         } else {
            $label = "$title";
         }
   ...
   ## apply logo only to personal calendars.
   if (($sc eq "p") && ($g eq "")) {
      if ($publogo ne "") {
         $prml = strapp $prml, "logo=$publogo";
      }
   }
   ...
   if ( ($sc eq "p") || ($publicedit eq "CHECKED")) {
...
# Init group cal link
       if ($g eq "") {
          ...
          $grpcal = adjusturl "<a href=\"http://$vdomain/cgi-bin/$cgirh/execgroupcal.cgi?biscuit=$biscuit&jp=$jp&os=$os\">Group Calendars</a> - ";
          ...
          $prml = strapp $prml, "groupcal=$grpcal";
           ...
           ORDER => ['groupname', 'groupfounder', 'grouptype', 'grouptitle',
                 ...
                 'listed' ] };
          $calline = adjusturl "<TABLE BORDER=1 CELLSPACING=0 CELLPADDING=2 WIDTH=\"90%\"><TR><TD><TABLE BGCOLOR=\"0f0f5f\" CELLPADDING=0 CELLSPACING=0 WIDTH=\"100%\" BORDER=0><TR><TD WIDTH=\"25%\"><FONT FACE=\"Verdana\" COLOR=ffffff SIZE=\"2\">$lgrouptab{$g}{'ctype'}&nbsp;</FONT></TD><TD ALIGN=\"CENTER\" WIDTH=\"50%\"><FONT COLOR=ffffff FACE=\"Verdana\" SIZE=\"2\">$lgrouptab{$g}{'grouptitle'}&nbsp;</FONT></TD><TD WIDTH=\"25%\"><FONT COLOR=ffffff FACE=\"Verdana\" SIZE=\"2\">$lgrouptab{$g}{'corg'}&nbsp;</FONT></TD></TR></TABLE></TD></TR></TABLE>";
          $prml = strapp $prml, "calline=$calline";
       }
...
# Init the rep directory
   ...
   if (!-d " $ENV{HDHREP}/$alphaindex/$login") {
      system "mkdir -p $ENV{HDHREP}/$alphaindex/$login";
      system "chown nobody:nobody $ENV{HDHREP}/$alphaindex/$login";
      system "chmod 755 $ENV{HDHREP}/$alphaindex/$login";
   }

   if (!-d " $ENV{HDREP}/$alphaindex/$login") {
      system "mkdir -p $ENV{HDHREP}/$alphaindex/$login";
      system "chown nobody:nobody $ENV{HDHREP}/$alphaindex/$login";
      system "chmod 755 $ENV{HDHREP}/$alphaindex/$login";
   }
...
# Init the hidden fields for Form processing
   ...
   if ($a eq "de") {
      ...
      if ($sc eq "p") {
        $prml = calfuncs::calfuncs::displayDetails($prml, $dy, $mo, $yr, $login, $url, $vw, $en, $f, $g);
      }
   } 
...
# this is redirect url, will be invoked when Save button is pressed
      #hddebug "just before rh = $rh";
         #hddebug "came HERE!!!!!!!!!!";
      if ($rh ne "") {
         #$cgirh = "http://$vdomain/cgi-bin/$rh/";
      }
      #$rurl = adjusturl "$cgirh$cgis&vw=w&mo=$mo&dy=$dy&yr=$yr&f=$f&a=d&jvw=$jvw";
      $rurl = adjusturl "$cgis&vw=w&mo=$mo&dy=$dy&yr=$yr&f=$f&a=d&jvw=$jvw";
      #hddebug "rurl = $rurl";
      #hddebug "cgis = $cgis";
...
# Initialize Functions
   ...
   $prml = strapp $prml, "home=$fref"; 
      ...
      #if ($f eq "e") {
         $repht = adjusturl "<a href=\"$cgis&mo=$mo&dy=$dy&yr=$yr&vw=$vew&f=r&a=d\">Report</a><BR><BR>";
...
# Initialize func
   if (($vw eq "d") || ($vw eq "m") || ($vw eq "w")) {
      if ($f eq "e") {
         $prml = strapp $prml, "func=Reminders";
      }
      if ($f eq "t") {
   ...
   if (($sc eq "p") || ($publicedit eq "CHECKED") ) {
     ...
     $prml = strapp $prml, "whatsnew=";
     $prml = strapp $prml, "features=";
     $prml = strapp $prml, "dash=";
   } else {
     $prml = strapp $prml, "whatsnew=What's New?";
...
# daily view
   if ($vw eq "d") {
	 ...
	 withdrawmoney $login;
         ...
         $prm = strapp $prm, "templateout=$ENV{HDHREP}/$alphaindex/$login/$biscuit-$$-redirect_url.html";
         parseIt $prm;
         hdsystemcat "$ENV{HDHREP}/$alphaindex/$login/$biscuit-$$-redirect_url.html";
	 ## this is for print 
         #$rurl = "$cgis&vw=$vw&mo=$mo&dy=$dy&yr=$yr&f=$f&a=d";
         #print "Location: $rurl\n\n";
...
# Init the day label
      ...
      if ( ($jp ne "") && (-f "$ENV{HDDATA}/$alphj/$jp/templates/dailycalendar.html") ) {
         #hddebug "jp = $jp, using new template";
         qx{cat $ENV{HDDATA}/$alphj/$jp/templates/dailycalendar.html > $tpartial};
      } else {
      ...
      if (($f eq "e") || ($f eq "t")) {
         ...
         for ($i = 0; $i <= 18; $i++) {
            ...
            if ($sc eq "p") {
               $timeref = adjusturl "$cgiscript&mo=$mo&dy=$dy&yr=$yr&vw=d&f=$f&a=d&h=$h&m=$m";
...
# monthly view
   if ($vw eq "m") {
	 ...
	 ## this is for print
      ...
      if ( ($jp ne "") && (-f "$ENV{HDDATA}/$alphj/$jp/templates/monthlycalendar.html") ) {
         qx{cat $ENV{HDDATA}/$alphj/$jp/templates/monthlycalendar.html > $tpartial};
      } else {
...
# weekly view
   if ($vw eq "w") {
	 ...
	 ## this is for print
      ...
      if ( ($jp ne "") && (-f "$ENV{HDDATA}/$alphj/$jp/templates/weeklycalendar.html") ) {
         qx{cat $ENV{HDDATA}/$alphj/$jp/templates/weeklycalendar.html > $tpartial};
      } else {
...
# destination template does not change, and is a parameter!
   ...
   parseIt $prml;
   if (! -f $dp) {
      hddebug "$dp is not created for some reason";
   }
}
