...
###
####  The only change is instead of using execcalclient.cgi execothercalendar.cgi
#### is used
### 
sub createCalendar {
   my($dy, $mo, $yr, $dp, $login, $biscuit, $vw, $f, $a, $h, $m, $tz, $en, $sc, $jvw, $g, $calp, $rh, $logo, $title, $banner, $vdomain, $hs, $jp, $os, $othermember) = @_;
...
# substitute HTTPSUBDIR
   ...
   $prml = strapp $prml, "othermember=$othermember";
...
# Init the base CGI url
   ...
   if ($biscuit eq "") {
       $cgiscript = adjusturl "execothercalendar.cgi?sc=$sc&l=$login&g=$g&jp=$jp&othermember=$othermember";
   } else {
       $cgiscript = adjusturl "execothercalendar.cgi?biscuit=$biscuit&g=$g&jp=$jp&othermember=$othermember";
   }
...
# Init the page header
   ...
   if ($g ne "") {
      ...
      if ($sc eq "p") {
         ...
         if ($calp  eq "CHECKED") {
            if ($hs eq "") {
               $label = adjusturl "$title<BR></b></FONT><FONT FACE=\"Verdana\" SIZE=\"2\">My Website (http://$vdomain/members/$alpha/$login)</FONT>";
            } else {
               $label = adjusturl "$title<BR></b></FONT><FONT FACE=\"Verdana\" SIZE=\"2\">My Website (http://$vdomain/$hs/members/$alpha/$login)</FONT>";
            }
         } else {
            $label = "$title";
         }
...
# Init group cal link
	  ...
	  $calline = "You are editing calendar of $othermember";
...
# Init master template vars
   ...
   $caltop = "$ENV{HDTMPL}/otherhdcalviewtop.html";
...
# Init the hidden fields for Form processing
   ...
   if ($a eq "de") {
      ...
      hddebug "came here to displayDetails";
      if ($sc eq "p") {
        $prml = calfuncs::calfuncs::displayDetails($prml, $dy, $mo, $yr, $othermember, $url, $vw, $en, $f, $g);
      }
   } 
...
# this is redirect url, will be invoked when Save button is pressed
      ...
      $prm = strapp $prm, "templateout=$ENV{HDHREP}/$alpha/$login/$biscuit-$$-redirect_url.html";
      parseIt $prm;
      $prml = strapp $prml, "rurl=$ENV{HDHREP}/$alpha/$login/$biscuit-$$-redirect_url.html";
   }
...
# Initialize Functions
   ...
   if ( ($login ne "") && ($sc ne "p") && (-f "$ENV{HDREP}/$alpha/$login/topcal.html") ) {
   ...
   $prml = strapp $prml, "home=$fref"; 
      ...
      #if ($f eq "e") {
         $repht = adjusturl "<a href=\"$cgis&vdomain=$vdomain&rh=$rh&hs=$hs&mo=$mo&dy=$dy&yr=$yr&vw=$vew&f=r&a=d\">Report</a><BR><BR>";

         #$repht = adjusturl "$cgis&vdomain=$vdomain&rh=$rh&hs=$hs&mo=$mo&dy=$dy&yr=$yr&vw=$vew&f=r&a=d";
...
# daily view
   if ($vw eq "d") {
	 ...
	 withdrawmoney $login;
	 hddebug "deleteEvent is called, f = $f, othermember = $othermember";
         if ($f eq "t") {
            deleteTodo($en, $othermember, $g);
         } else {
            deleteEvent($en, $othermember, $g);
         }
         ...
         $prm = strapp $prm, "templateout=$ENV{HDHREP}/$alpha/$login/$biscuit-$$-redirect_url.html";
         parseIt $prm;
         system "/bin/cat $ENV{HDHREP}/$alpha/$login/$biscuit-$$-redirect_url.html";
         #$prml = strapp $prml, "rurl=$ENV{HDHREP}/$alpha/$login/$biscuit-$$-redirect_url.html";
         exit;
      }
...
# Init the day label
      ...
      $tpartial = "$hdhrep/otherhdcaldailyview-$biscuit.html";
...
# new item view
   if (($vw eq "n") || ($vw eq "i")) {
      ...
      if (($f eq "e") && ($a eq "da")) {
         $tpartial = "$hdhrep/otherhdcaleventview-$biscuit.html";
      ...
      if (($f eq "t") && ($a eq "da")) {
         $tpartial = "$hdhrep/otherhdcaltodoview-$biscuit.html";
...
# monthly view
   if ($vw eq "m") {
      ...
      $tpartial = "$hdhrep/otherhdcalmonthlyview-$biscuit.html";
...
# yearly view
   ...
   if (($yr >= 1996) && ($yr <= 2008)) {
      #hddebug "came here";
      $tpartial = "$hdhrep/otherhdcalyearlyview-$biscuit.html";
...
# weekly view
   if ($vw eq "w") {
      ...
      $tpartial = "$hdhrep/otherhdcalweeklyview-$biscuit.html";
...
# destination template does not change, and is a parameter!
   ...
   if (($a ne "") && ($vw ne "y") && ($a ne "da")) {
      #if (($a eq "d") && ($login eq "smitha")) {
      #   $prml = displayEventsAndTodos $prml, $dy, $mo, $yr, $login, $url, $vw, $f, $jvw, $g, $sc; 
      #} else {
	  hddebug "dispatch() othermember = $othermember";
         $prml = dispatch $prml, $vw, $f, $a, $mo, $dy, $yr, $h, $m, $othermember, $url, $en, $jvw, $sc, $g;
      #}
   }
