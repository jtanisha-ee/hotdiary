package businesscalutil::teammergedcalutil;
require Exporter;
use Time::Local;
use tparser::tparser;
use utils::utils;
use calfuncs::mergedviewfuncs;
...
sub createCalendar {
   ...
   if ($os ne "nt") {
      $execmergedcalclient = encurl "execmergedcalclient.cgi";	
   } else {
      $execmergedcalclient = "execmergedcalclient.cgi";	
   }

   if ($f ne "m") {
      $hiddenvars = "<INPUT TYPE=HIDDEN NAME=p0 VALUE=\"$execmergedcalclient\">";
   } 
...
# Init the base CGI url
   ...
   if ($biscuit eq "") {
       ...
       $cgiscript = "execdogeneric.cgi?pnum=12&p0=$execmergedcalclient&p1=sc&p2=l&p3=teamname&p4=business&p5=mo&p6=dy&p7=yr&p8=vw&p9=f&p10=a&p11=jp&re0=CGISUBDIR&le0=rh&re1=HTTPSUBDIR&le1=hs&re2=SERVER_NAME&le2=vdomain&re3=HDLIC&le3=HDLIC&HDLIC=$HDLIC&$dpms&le4=os&re4=os&enum=5";
       #$cgiscript = adjusturl "execmergedcalclient.cgi?sc=$sc&l=$login&g=$g&jp=$jp";
      
   } else {
       $dpms = "biscuit=$biscuit&teamname=$teamname&jp=$jp&business=$business";
       $cgiscript = "execdogeneric.cgi?pnum=13&p0=$execmergedcalclient&p1=biscuit&p2=teamname&p3=business&p4=mo&p5=dy&p6=yr&p7=vw&p8=f&p9=a&p10=jp&p11=jvw&p12=en&re0=CGISUBDIR&le0=rh&re1=HTTPSUBDIR&le1=hs&re2=SERVER_NAME&le2=vdomain&re3=HDLIC&le3=HDLIC&HDLIC=$HDLIC&$dpms&le4=os&re4=os&enum=5";
       #$cgiscript = adjusturl "execmergedcalclient.cgi?biscuit=$biscuit&g=$g&jp=$jp";
...
# Init the page header
   ...
   $label2 = "Use Events, To-Dos, Journal (Merged view of Personal Calendars in a Team) ";
...
# Init group cal link
       if ($teamname eq "") {
          ...
          $calline = adjusturl "<TABLE CELLPADDING=0 CELLSPACING=0 WIDTH=\"100%\" BORDERWIDTH=0><TR><TD WIDTH=\"15%\"><FONT FACE=\"Verdana\" SIZE=\"2\">Team Title - $teamtab{$teamname}{teamtitle}&nbsp;</FONT></TD><TD ALIGN=\"CENTER\" WIDTH=\"70%\"><FONT FACE=\"Verdana\" SIZE=\"2\">TeamName - $teamname&nbsp;</FONT></TD><TD WIDTH=\"15%\"><FONT FACE=\"Verdana\" SIZE=\"2\">Business - $business &nbsp;</FONT></TD></TR></TABLE>";
...
# Init master template vars
   ...
   $caltop = "$ENV{HDTMPL}/mergedviewtop.html";
   $calbot = "$ENV{HDTMPL}/hdbusinesscalviewbottom.html";
   $caldaily = "$ENV{HDTMPL}/mergeddailyview.html";
...
# Init the hidden fields for Form processing
   ...
   if ($a eq "de") {
      ...
      $prml = calfuncs::mergedviewfuncs::displayDetails($prml, $dy, $mo, $yr, $login, $url, $vw, $en, $f, $business, $teamname);  
...
# this is redirect url, will be invoked when Save button is pressed
      ...
      if ($biscuit eq "") {
         $rscript = "execdogeneric.cgi?pnum=13&p0=$execmergedcalclient&p1=sc&p2=l&p3=teamname&p4=business&p5=mo&p6=dy&p7=yr&p8=vw&p9=f&p10=a&p11=jp&p12=jvw&re0=CGISUBDIR&le0=rh&re1=HTTPSUBDIR&le1=hs&re2=SERVER_NAME&le2=vdomain&re3=HDLIC&le3=HDLIC&HDLIC=$HDLIC&$dpms&le4=os&re4=os&enum=5";
      } else {
         $rscript = "execdogeneric.cgi?pnum=12&p0=$execmergedcalclient&p1=biscuit&p2=teamname&p3=business&p4=mo&p5=dy&p6=yr&p7=vw&p8=f&p9=a&p10=jp&p11=jvw&re0=CGISUBDIR&le0=rh&re1=HTTPSUBDIR&le1=hs&re2=SERVER_NAME&le2=vdomain&re3=HDLIC&le3=HDLIC&HDLIC=$HDLIC&$dpms&le4=os&re4=os&enum=5";
      }
...
# Initialize func
   if (($vw eq "d") || ($vw eq "m") || ($vw eq "w")) {
      if ($f eq "e") {
         $prml = strapp $prml, "func=Events & To-Dos";
      }
      if ($f eq "t") {
...
# daily view
   if ($vw eq "d") {
      ...
      if ($a eq "r") {
         ...
         #if ($f eq "t") {
	    #status("$login: You cannot delete events/todos from merged view of personal calendars in a team.");
	    #exit; 
...
# Init the day label
      ...
      if (($f eq "e") || ($f eq "t") || ($f eq "m")) {
         ...
         for ($i = 0; $i <= 18; $i++) {
            ...
            if ($biscuit eq "") {
                $tscript = "execdogeneric.cgi?pnum=14&p0=$execmergedcalclient&p1=sc&p2=l&p3=teamname&p4=business&p5=mo&p6=dy&p7=yr&p8=vw&p9=f&p10=a&p11=jp&p12=h&p13=m&re0=CGISUBDIR&le0=rh&re1=HTTPSUBDIR&le1=hs&re2=SERVER_NAME&le2=vdomain&re3=HDLIC&le3=HDLIC&HDLIC=$HDLIC&$dpms&le4=os&re4=os&enum=5";
            } else {
                $tscript = "execdogeneric.cgi?pnum=13&p0=$execmergedcalclient&p1=biscuit&p2=teamname&p3=business&p4=mo&p5=dy&p6=yr&p7=vw&p8=f&p9=a&p10=jp&p11=h&p12=m&re0=CGISUBDIR&le0=rh&re1=HTTPSUBDIR&le1=hs&re2=SERVER_NAME&le2=vdomain&re3=HDLIC&le3=HDLIC&HDLIC=$HDLIC&$dpms&le4=os&re4=os&enum=5";
            }
...
# destination template does not change, and is a parameter!
   ...
   if (($a ne "") && ($vw ne "y") && ($a ne "da")) {
      $prml = calfuncs::mergedviewfuncs::dispatch $prml, $vw, $f, $a, $mo, $dy, $yr, $h, $m, $login, $url, $en, $jvw, $sc, $business, $teamname;
   }
   #status("prml = $prml");
   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=pnum VALUE=$hiddenpnum>";
   $hiddenvars = adjusturl $hiddenvars;
