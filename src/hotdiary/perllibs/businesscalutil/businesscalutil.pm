package businesscalutil::businesscalutil;
require Exporter;
use Time::Local;
use tparser::tparser;
use utils::utils;
...
@ISA = qw(Exporter);
@EXPORT = qw(createCalendar nextMonth yrOfNextMonth prevMonth getweekendday yrOfPrevMonth getDateNumSuffix getmonth getday isDayInWeek getWeekDayIndex isdsttime getdaystr getAdjustedDay getDayOfWeek);
...
sub createCalendar {
   my($dy, $mo, $yr, $dp, $login, $biscuit, $vw, $f, $a, $h, $m, $tz, $en, $sc, $jvw, $teamname, $calp, $rh, $logo, $title, $banner, $vdomain, $hs, $jp, $os, $business, $partner) = @_;

   hddebug "businesscalutil(), teamname = $teamname";
...
# test generic functionality
   if ($login eq "manoj") {
      $showbiz = adjusturl "<a href=\"/cgi-bin/$rh/execdogeneric.cgi?pnum=3&p0=:$execfeature:&p1=fcomp&p2=biscuit&fcomp=manoj&biscuit=$biscuit&le3=os&re3=os&enum=4&re0=CGISUBDIR&le0=rh&re1=HTTPSUBDIR&le1=hs&re2=SERVER_NAME&le2=vdomain\">Feature</a><BR><BR>";
...
# Init the base CGI url
   ...
   if ($biscuit eq "") {
       ...
       $cgiscript = "execdogeneric.cgi?pnum=12&p0=$execbusinesscalclient&p1=sc&p2=l&p3=teamname&p4=business&p5=mo&p6=dy&p7=yr&p8=vw&p9=f&p10=a&p11=jp&re0=CGISUBDIR&le0=rh&re1=HTTPSUBDIR&le1=hs&re2=SERVER_NAME&le2=vdomain&re3=HDLIC&le3=HDLIC&HDLIC=$HDLIC&$dpms&le4=os&re4=os&enum=5";
       #$cgiscript = adjusturl "execbusinesscalclient.cgi?sc=$sc&l=$login&g=$g&jp=$jp";
      
   } else {
       $dpms = "biscuit=$biscuit&teamname=$teamname&jp=$jp&business=$business";
       $cgiscript = "execdogeneric.cgi?pnum=13&p0=$execbusinesscalclient&p1=biscuit&p2=teamname&p3=business&p4=mo&p5=dy&p6=yr&p7=vw&p8=f&p9=a&p10=jp&p11=jvw&p12=en&re0=CGISUBDIR&le0=rh&re1=HTTPSUBDIR&le1=hs&re2=SERVER_NAME&le2=vdomain&re3=HDLIC&le3=HDLIC&HDLIC=$HDLIC&$dpms&le4=os&re4=os&enum=5";
       #$cgiscript = adjusturl "execbusinesscalclient.cgi?biscuit=$biscuit&g=$g&jp=$jp";
...
# Init group cal link
       if ($teamname eq "") {
          ...
          $calline = "Team $teamname";
       } else {
          if ($rh ne "") {
             $cgirh = "$rh";
          }
          $calline = adjusturl "<TABLE CELLPADDING=0 CELLSPACING=0 WIDTH=\"100%\" BORDERWIDTH=0><TR><TD WIDTH=\"15%\"><FONT FACE=\"Verdana\" SIZE=\"2\">$teamtab{$teamname}{teamtitle}&nbsp;</FONT></TD><TD ALIGN=\"CENTER\" WIDTH=\"70%\"><FONT FACE=\"Verdana\" SIZE=\"2\">$teamname&nbsp;</FONT></TD><TD WIDTH=\"15%\"><FONT FACE=\"Verdana\" SIZE=\"2\">$business&nbsp;</FONT></TD></TR></TABLE>";
...
# Init master template vars
   ...
   $caltop = "$ENV{HDTMPL}/hdbusinesscalviewtop.html";
   $calbot = "$ENV{HDTMPL}/hdbusinesscalviewbottom.html";
   $caldaily = "$ENV{HDTMPL}/hdbusinesscaldailyview.html";
   $calyearly = "$ENV{HDTMPL}/hdbusinesscalyearlyview.html";
   $calmonthly = "$ENV{HDTMPL}/hdbusinesscalmonthlyview.html";
   $calweekly = "$ENV{HDTMPL}/hdbusinesscalweeklyview.html";

   if ($sc eq "p") {
      $calevent = "$ENV{HDTMPL}/showevent.html";
      $caltodo = "$ENV{HDTMPL}/showevent.html";
   } else {
      $calevent = "$ENV{HDTMPL}/hdbusinesscaleventview.html";
      $caltodo = "$ENV{HDTMPL}/hdbusinesscaltodoview.html";
   }
...
# Init the hidden fields for Form processing
   ...
   if ($a eq "de") {
      ...
      if ($sc eq "p") {
	 $prml = calfuncs::businesscalfuncs::displayDetails($prml, $dy, $mo, $yr, $login, $url, $vw, $en, $f, $business, $teamname);
      }
...
# this is redirect url, will be invoked when Save button is pressed
      ...
      if (($f eq "e") && ($a eq "de")) {
         ...
         $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p33 VALUE=spc>";
         $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p34 VALUE=save>";
         $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p35 VALUE=evtbanner>";
         $hiddenpnum = 36;
      }
      if (($f eq "t") && ($a eq "de")) {
         $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p15 VALUE=subject>";
         ...
         $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p26 VALUE=save>";
         $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p27 VALUE=spc>";
         $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p28 VALUE=evtbanner>";
         $hiddenpnum = 29;
      }
      if (($f eq "m") && ($a eq "de")) {
      ...
      if ($biscuit eq "") {
         $rscript = "execdogeneric.cgi?pnum=13&p0=$execbusinesscalclient&p1=sc&p2=l&p3=teamname&p4=business&p5=mo&p6=dy&p7=yr&p8=vw&p9=f&p10=a&p11=jp&p12=jvw&re0=CGISUBDIR&le0=rh&re1=HTTPSUBDIR&le1=hs&re2=SERVER_NAME&le2=vdomain&re3=HDLIC&le3=HDLIC&HDLIC=$HDLIC&$dpms&le4=os&re4=os&enum=5";
      } else {
         $rscript = "execdogeneric.cgi?pnum=12&p0=$execbusinesscalclient&p1=biscuit&p2=teamname&p3=business&p4=mo&p5=dy&p6=yr&p7=vw&p8=f&p9=a&p10=jp&p11=jvw&re0=CGISUBDIR&le0=rh&re1=HTTPSUBDIR&le1=hs&re2=SERVER_NAME&le2=vdomain&re3=HDLIC&le3=HDLIC&HDLIC=$HDLIC&$dpms&le4=os&re4=os&enum=5";
      }
...
# Initialize Functions
   ...
   if ( ($login ne "") && ($sc ne "p") && (-f "$ENV{HDREP}/$alphaindex/$login/topcal.html")
      ...
      ## this goes to manage teams
          $fref = adjusturl "/cgi-bin/$rh/execdogeneric.cgi?p0=ZXhlY3Nob3didXNpbmVzc2NhbG1lbnUuY2dp&pnum=3&p1=biscuit&p2=businesslist&re0=CGISUBDIR&le0=rh&re1=HTTPSUBDIR&le1=hs&re2=SERVER_NAME&le2=vdomain&re3=HDLIC&le3=HDLIC&le4=os&re4=os&biscuit=$biscuit&enum=5&businesslist=$business";
   } else {
      $fref = adjusturl "$cgis&mo=$mo&dy=$dy&yr=$yr&vw=$vew&f=h&a=d";
   }                 
...
# daily view
   if ($vw eq "d") {
      ...
      if ($a eq "r") {
         ...
         if ($f eq "t") {
            deleteTodo($en, $login, $business, $teamname);
         } else {
            deleteEvent($en, $login, $business, $teamname);
         }
...
# Init the day label
      ...
      $tpartial = "$hdhrep/hdbusinesscaldailyview-$biscuit.html";
      ...
      if (($f eq "e") || ($f eq "t") || ($f eq "m")) {
         ...
         for ($i = 0; $i <= 18; $i++) {
            ...
            if ($biscuit eq "") {
                $tscript = "execdogeneric.cgi?pnum=14&p0=$execbusinesscalclient&p1=sc&p2=l&p3=teamname&p4=business&p5=mo&p6=dy&p7=yr&p8=vw&p9=f&p10=a&p11=jp&p12=h&p13=m&re0=CGISUBDIR&le0=rh&re1=HTTPSUBDIR&le1=hs&re2=SERVER_NAME&le2=vdomain&re3=HDLIC&le3=HDLIC&HDLIC=$HDLIC&$dpms&le4=os&re4=os&enum=5";
            } else {
                $tscript = "execdogeneric.cgi?pnum=13&p0=$execbusinesscalclient&p1=biscuit&p2=teamname&p3=business&p4=mo&p5=dy&p6=yr&p7=vw&p8=f&p9=a&p10=jp&p11=h&p12=m&re0=CGISUBDIR&le0=rh&re1=HTTPSUBDIR&le1=hs&re2=SERVER_NAME&le2=vdomain&re3=HDLIC&le3=HDLIC&HDLIC=$HDLIC&$dpms&le4=os&re4=os&enum=5";
            }
...
# new item view
   if (($vw eq "n") || ($vw eq "i")) {
      ...
      if (($f eq "e") && ($a eq "da")) {
         $tpartial = "$hdhrep/hdbusinesscaleventview-$biscuit.html";
      ...
      if (($f eq "t") && ($a eq "da")) {
         $tpartial = "$hdhrep/hdbusinesscaltodoview-$biscuit.html";
         ...
         $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p24 VALUE=status>";
         $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p25 VALUE=share>";
         $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p26 VALUE=spc>";
         $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p27 VALUE=save>";
         $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p28 VALUE=evtbanner>";
         $hiddenpnum = 29;
      } 
...
# monthly view
   if ($vw eq "m") {
      ...
      $tpartial = "$hdhrep/hdbusinesscalmonthlyview-$biscuit.html";
...
# yearly view
   ...
   if (($yr >= 1996) && ($yr <= 2008)) {
      $tpartial = "$hdhrep/hdbusinesscalyearlyview-$biscuit.html";
...
########
      ...
      $prml = strapp $prml, "year=$yr"; 
      #$prml = strapp $prml, "template=$ENV{HDTMPL}/hdbusinesscalyearlyview.html"; 
   }
   }
...
# weekly view
   if ($vw eq "w") {
      ...
      $tpartial = "$hdhrep/hdbusinesscalweeklyview-$biscuit.html";
...
# destination template does not change, and is a parameter!
   ...
   if (($a ne "") && ($vw ne "y") && ($a ne "da")) {
	$part_ner = "iir";
        ...
        #$prml = calfuncs::businesscalfuncs::displayEventsAndTodos $prml, $dy, $mo, $yr, $login, $url, $vw, $f, $jvw, $business, $sc, $teamname;
     #} else {
        $prml = calfuncs::businesscalfuncs::dispatch $prml, $vw, $f, $a, $mo, $dy, $yr, $h, $m, $login, $url, $en, $jvw, $sc, $business, $teamname;
     #}
   }
   #status("prml = $prml");
   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=pnum VALUE=$hiddenpnum>";
