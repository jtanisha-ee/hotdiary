...
#session timeout in secs
   ...
   ## fp indicates whether it is public
   if ($fp eq "") {
     if ($biscuit eq "") {
      if ($hs eq "") {
           if ($jp ne "") {
              if ($jp ne "buddie") {
                status("Your login session information is missing. Perhaps you are simply viewing a published carry on website. Click <a href=\"http://$vdomain/$icgi\"> here</a> to login.\n");
                exit;
              }
           }
           status("Your login session information is missing. Perhaps you are simply viewing a published carryon website. Click <a href=\"http://$vdomain/$icgi\" TARGET=\"_parent\"> here</a> to login.\n");
        } else {

           status("Your login session information is missing. Perhaps you are simply viewing a published carryon website. Click <a href=\"http://$vdomain/$hs/$icgi\" TARGET=\"_parent\"> here</a> to login.\n");
        }
        exit;
     }
   }
...
# bind login table vars
   ...
   $msg = "Click <a href=\"http://www.hotdiary.com/cgi-bin/execfilebrowser.cgi?flg=$newflg&framedomain=$framedomain&jp=$jp&re0=CGISUBDIR&le0=rh&re1=HTTPSUBDIR&le1=hs&re2=SERVER_NAME&le2=vdomain&re3=HDLIC&le3=HDLIC&HDLIC=$HDLIC&biscuit=$biscuit&f=mc&rh=$rh&hs=$hs&vdomain=$vdomain&le4=os&re4=os&re5=HTTP_COOKIE&le5=HTTP_COOKIE&enum=6\"><b>here</a></b> to go back to file Carry-On'.";

   if (notCarryOnFile($currdir)) {
      status("login: You can have only alphabets(a-z)(A-Z), numerals(0-9), underscore(_), hyphen(-), forwards slash(/), and a dot(.) in your directory path. $msg");
      exit;
   }
   ...
   while (<infiile>) {
	 ...
	 $dirpath = "$jumpdir";
	    ...
	    hddebug "ksize = $ksize";
            $list .= "<TD><a href=\"http://www.hotdiary.com/cgi-bin/execfilebrowser.cgi?flg=$newflg&framedomain=$framedomain&rh=$rh&hs=$hs&vdomain=$vdomain&jp=$jp&HDLIC=$HDLIC&biscuit=$biscuit&basedir=$jumpdir&currdir=$_\"><IMG SRC=\"$images/folder.gif\" WIDTH=20 HEIGHT=20 BORDER=0>$_</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</TD>";
	    ...
	    if (("\L$_" eq "\LMyFolder") || ("\L$_" eq "\LMyPhotoAlbum")) {
	       ...
	       $list .= "<TD><a href=\"http://www.hotdiary.com/cgi-bin/execeditfile.cgi?jp=$jp&framedomain=$framedomain&rh=$rh&hs=$hs&vdomain=$vdomain&jp=$jp&HDLIC=$HDLIC&login=$login&biscuit=$biscuit&en=$en&filename=$_&basedir=$jumpdir\">Change</TD>";
	       $list .= "<TD>$mon/$mday/$year &nbsp;</TD>";
	       $list .= "<TD><a href=\"http://www.hotdiary.com/cgi-bin/execdelfile.cgi?framedomain=$framedomain&jp=$jp&rh=$rh&hs=$hs&vdomain=$vdomain&jp=$jp&HDLIC=$HDLIC&dir=dir&dirpath=$dirpath&l=$login&biscuit=$biscuit&dirname=$_\"><IMG SRC=\"$images/delevt.gif\" WIDTH=20 HEIGHT=20 BORDER=0></a></TD>";
	       $list .= "<TD>N/A &nbsp;</TD></TR>";
	    }
         } else {
	    ...
	    ## syntax for file opening with appropriateextension
            ## file:/tmp/test.html
            $list .= "<TD><a href=\"http://www.hotdiary.com/cgi-bin/execfire.cgi?framedomain=$framedomain&rh=$rh&hs=$hs&vdomain=$vdomain&jp=$jp&HDLIC=$HDLIC&login=$login&biscuit=$biscuit&ftype=$ext&dfile=$jumpdir&catfile=$_\"><IMG SRC=\"$images/$fnimg\" WIDTH=20 HEIGHT=20 BORDER=0>$_</a>&nbsp;&nbsp;&nbsp;&nbsp&nbsp;</TD>";
	    $list .= "<TD>File</TD>";
	    $list .= "<TD>$size &nbsp;</TD>";
	    $list .= "<TD>$permission &nbsp;</TD>";
	    $list .= "<TD><a href=\"http://www.hotdiary.com/cgi-bin/execeditfile.cgi?framedomain=$framedomain&rh=$rh&hs=$hs&vdomain=$vdomain&jp=$jp&HDLIC=$HDLIC&login=$login&biscuit=$biscuit&en=$en&filename=$_&basedir=$jumpdir\">Change</TD>";
	    $list .= "<TD>$mon/$mday/$year &nbsp;</TD>";
	    $list .= "<TD><a href=\"http://www.hotdiary.com/cgi-bin/execdelfile.cgi?framedomain=$framedomain&rh=$rh&hs=$hs&vdomain=$vdomain&jp=$jp&HDLIC=$HDLIC&dir=file&dirpath=$dirpath&l=$login&biscuit=$biscuit&filename=$_\"><IMG SRC=\"$images/delevt.gif\" WIDTH=20 HEIGHT=20 BORDER=0></a></TD>";
	    $list .= "<TD><a href=\"http://www.hotdiary.com/cgi-bin/execcarryonmail.cgi?framedomain=$framedomain&rh=$rh&hs=$hs&vdomain=$vdomain&jp=$jp&HDLIC=$HDLIC&basedir=$jumpdir&l=$login&biscuit=$biscuit&filename=$_\">Email&nbsp;</a></TD></TR>";
         }   
	}
      }
   ...
   $list .= "</TABLE>";
   $list = adjusturl $list;
   $myfolder =  "<a href=\"http://www.hotdiary.com/cgi-bin/execfilebrowser.cgi?flg=$newflg&framedomain=$framedomain&rh=$rh&hs=$hs&vdomain=$vdomain&jp=$jp&HDLIC=$HDLIC&biscuit=$biscuit\">My Carry-On</a>";
