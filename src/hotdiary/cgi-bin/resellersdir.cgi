...
# bind login table vars
   ...
   foreach $mem (sort keys %resellertab) {
      $msg = "<TR>";
      $cntr = $cntr +1;
      $cdir .= $mem;
      $cdir .= " ";
      $msg .= "<TD><FONT FACE=\"Verdana\" SIZE=\"2\">$resellertab{$mem}{'fname'}&nbsp;$resellertab{$mem}{'lname'} &nbsp;</FONT></TD>";
      $email = $resellertab{$mem}{email};
      $msg .= "<TD><FONT FACE=\"Verdana\" SIZE=\"2\"><a href=\"mailto:$email\">$email</a> &nbsp;</FONT></TD>";
      $msg .= "<TD><FONT FACE=\"Verdana\" SIZE=\"2\">$resellertab{$mem}{'bphone'}&nbsp;</FONT></TD>";
      $dbpagertype = $resellertab{$mem}{pagertype};
      $dbpager = $resellertab{$mem}{pager};
      $uname = $resellertab{$mem}{fname}. " ". $resellertab{$mem}{lname}; 
      ($dbfax = $resellertab{$mem}{'fax'}) =~ s/\n/\n<BR>/g;
      $tostreet = replaceblanks $resellertab{$mem}{street};
      $tocity = replaceblanks $resellertab{$mem}{city};
      $tostate = replaceblanks $resellertab{$mem}{state};
   ...
   foreach $cn (@hshcdir) {
      ...
      if ($os ne "nt") {
         ...
         $execresellerdir = encurl "execresellerdir.cgi";
         $execproxylogout = encurl "/proxy/execproxylogout.cgi";
         $execdeploypage =  encurl "execdeploypage.cgi";
         $execshowtopcal =  encurl "execshowtopcal.cgi";
      } else {
         $prb = strapp $prb, "formenc=";
         $execresellerdir = "execresellerdir.cgi";
         $execproxylogout =  "/proxy/execproxylogout.cgi";
         $execdeploypage =  "execdeploypage.cgi";
         $execshowtopcal =  "execshowtopcal.cgi";
      }
      $prb = strapp $prb, "template=$ENV{HDTMPL}/resellerdir.html";
      $prb = strapp $prb, "templateout=$ENV{HDHREP}/$login/resellerdir.html";
      ...
      $hiddenvars = "<INPUT TYPE=HIDDEN NAME=p0 VALUE=\"$execresellerdir\">";
	 ...
	 $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=box$k VALUE=$cn>";
      ...
      $bizlabel = "$business - Resellers Directory"; 
      ...
      system "cat $ENV{HDHREP}/$login/resellerdir.html";
