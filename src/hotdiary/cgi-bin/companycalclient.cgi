...
#
# FileName: companycalclient.cgi
# Purpose: company wide calendar.
...
# reset the timer.
   ...
   if ($sc ne "p") {
      if (!exists($peopletab{$login})) {
         status("$login: You donot have permission to access $business company calendar.");
         exit; 
      }
   }
   $a = $input{a};

   if (!-d ( "$ENV{HDDATA}/business/business/$business/company/appttab")) {
   ...
   if (($sc ne "p") && ($a eq "u")) {
      companycalutil::companycalutil::addCompanyEventToMyCal($en, $login, $business);
      $a = "d";
   }
   ...
   if ($login ne $businesstab{$business}{businessmaster}) {
     ...
     if (!exists($managertab{$login})) {
       ...
       if (exists($managertab{$login})) {
          if ($mgraccesstab{access}{pother} ne "CHECKED") {
            $permit = 1;
         }
       }
     }
...
# Accessing hidden fields in form (hdcalviewtop.html)
      ...
      if ($f eq "e") {
         if ($a eq "a") {
            companycalutil::companycalfuncs::addEvent $etitle, $edtype, $edesc, $emonth, $eday, $eyear, $erecurtype, $ehour, $emin, $emeridian, $ezone, $eshare, $efree, $eatype, $edhour, $edmin, $login, $business, $econtact;
         }
         if ($a eq "e") {
            companycalutil::companycalfuncs::updateEvent $en, $etitle, $edtype, $edesc, $emonth, $eday, $eyear, $erecurtype, $ehour, $emin, $emeridian, $ezone, $eshare, $efree, $eatype, $edhour, $edmin, $login, $business, $econtact;
         }
         #system "cat $rurl";
         hdsystemcat "$rurl";
      } 
...
# Accessing hidden fields in form (hdcalviewtop.html)
      ...
      #status("rurl = $rurl");
      if ($a eq "a") {
         companycalutil::companycalfuncs::addTodo $etitle, $edesc, $emonth, $eday, $eyear, $ehour, $emeridian, $eshare, $epriority, $estatus, $login, $business;
      }
      if ($a eq "e") {
         companycalutil::companycalfuncs::updateTodo $en, $etitle, $edesc, $emonth, $eday, $eyear, $ehour, $emeridian, $eshare, $epriority, $estatus, $login, $business;
      }
      #system "cat $rurl";
      hdsystemcat "$rurl";
   ...
   #if ($login eq "smitha") {
      if ($f eq "r") {
         system "echo '#!/bin/ksh' > $ENV{HDHOME}/tmp/companyreportcal$$-$login";
         $repparms = "biscuit=$biscuit&jp=$jp&vdomain=$vdomain&hs=$hs&mo=$mo&yr=$yr&rh=$rh&HDLIC=$ip";
         $cgis = "$ENV{HDEXECCGI}/execcompanyreportcal.cgi";
         system "echo \"$cgis\" \"$repparms\" >> $ENV{HDHOME}/tmp/companyreportcal$$-$login";
         #$cgis = "$ENV{HDEXECCGI}/execcompanyreportcal.cgi?biscuit=$biscuit&jp=$jp&vdomain=$vdomain&hs=$hs&company=$business&mo=$mo&yr=$yr&rh=$rh&HDLIC=$ip&business=$business";
         # reset the timer.
         if (-f "$ENV{HDHOME}/tmp/companyreportcal$$-$login") {
    	    system "chmod 777 $ENV{HDHOME}/tmp/companyreportcal$$-$login";
            $sesstab{$biscuit}{'time'} = time();
            system "\"$ENV{HDHOME}/tmp/companyreportcal$$-$login\"";
         } else {
   ...
   if (($a ne "a") && ($a ne "e")) {
      ...
      #$publish = $businesstab{$business}{cpublish};

      hddebug "called createcompanycalendar, repdir = $repdir"; 
      companycalutil::companycalutil::createCompanyCalendar($dy, $mo, $yr, "$ENV{HDHREP}/$repdir/companycal$rand$biscuit.html", $login, $biscuit, $vw, $f, $a, $h, $m, $tz, $en, $sc, $jvw, $publish, "$rh", "$logo", "$title", "$banner", "$vdomain", "$hs", "$jp", "$os", "$business", $partner);
      
      #system "cat \"$ENV{HDHREP}/$repdir/companycal$rand$biscuit.html\"";
      hdsystemcat "$ENV{HDHREP}/$repdir/companycal$rand$biscuit.html";
   }
