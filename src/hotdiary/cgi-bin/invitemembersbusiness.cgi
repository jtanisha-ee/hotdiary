...
#
# FileName: invitemembersbusiness.cgi
...
#session timeout in secs
   ...
   #print &PrintHeader;
   #print &HtmlTop ("invitemembersbusiness.cgi example");
...
# bind login table vars
   ...
   if ($login ne $businesstab{$business}{businessmaster}) {
	 ...
	 system "chmod 755 $ENV{HDDATA}/business/business/$business/otheraccesstab";
      ...
      if (!exists($otheraccesstab{access})) {
	 $otheraccesstab{access}{access} = "";
      ...
      if ($otheraccesstab{access}{access} ne "CHECKED") {
         if (!exists($managertab{$login})) {
            status("$login: You do not have the permission to invite people. Manager role & Other role settings donot allow to invite people to business.  <BR>Click <a href=\"http://$vdomain/cgi-bin/$rh/execdogeneric.cgi?pnum=3&p0=$execbusiness&p1=biscuit&p2=jp&re0=CGISUBDIR&le0=rh&re1=HTTPSUBDIR&le1=hs&re2=SERVER_NAME&le2=vdomain&re3=HDLIC&le3=HDLIC&HDLIC=$HDLIC&biscuit=$biscuit&rh=$rh&jp=$jp&hs=$hs&vdomain=$vdomain&le4=os&re4=os&re5=HTTP_COOKIE&le5=HTTP_COOKIE&enum=6\">here</a> to return to business home."); 
            exit;
	 }
      }
   }
     
   hddebug "business (invitemembersbusiness.cgi) = $business";
   ...
   $prml = strapp $prml, "template=$ENV{HDTMPL}/invitemembersbusiness.html";
   $prml = strapp $prml, "templateout=$ENV{HDHREP}/$alphaindex/$login/invitemembersbusiness-$$.html";
   ...
   $prml = strapp $prml, "execinvitebynamebusiness=$execinvitebynamebusiness";
   $prml = strapp $prml, "execinvitebyloginbusiness=$execinvitebyloginbusiness";
   $prml = strapp $prml, "jp=$jp";

   parseIt $prml;

   #system "cat $ENV{HDTMPL}/content.html";
   #system "cat $ENV{HDHREP}/$alphaindex/$login/invitemembersbusiness.html";
   hdsystemcat "$ENV{HDHREP}/$alphaindex/$login/invitemembersbusiness-$$.html";
