...
# bind cntrtab table vars
   ...
   SCHEMA => {
    ...
    #foreach $fl (%dirlistt) {
    $mycnt = 0;
    system "echo \"*******************************\" >> /var/tmp/crontest";
    system "echo \"*******************************\" >> /var/tmp/crontest";
    system "echo \"Invoked cron reminder, PID = $$\" >> /var/tmp/crontest";
    $lult = localtime(time());
    system "echo \"Current Time = $lult\" >> /var/tmp/crontest";
    foreach $fl (keys %logtab) {
      $mycnt += 1;
      #print "entered for each loop fl = $fl\n";
      ($lg, $sf) = split(/\./, $fl);
      if ($lg eq "manoj") {
         system "echo \"User = $lg\" >> /var/tmp/crontest";
      }
	...
	#hddebug "entered while loop \n";
	    ...
	    if (exists $appttab{$onekey}) { 
	       #print "entered exists condition \n";
            ...
            if ($lg eq "manoj") {
               system "echo \"($onekey): $lg, $appthour:$apptmin $meridian, $apptday:$apptmonth:$apptyear\" >> /var/tmp/crontest";
            }
	    ...
	    $recurtype = $appttab{$onekey}{'recurtype'};
            ...
            $ctime = ctimetosec();
            if ($lg eq "manoj")  {
               system "echo \"ctime:$ctime\" >> /var/tmp/crontest";
            }
            $etime = 0;
	    #print "ctime = $ctime \n";
            ...
            if ($appttab{$onekey}{'recurtype'} ne "Once") {
                ...
                system "echo \"recurtype:$recurtype\" >> /var/tmp/crontest";
                system "echo \"onekey:$onekey\" >> /var/tmp/crontest";
                system "echo \"gmcur_mon, $gmcur_mon\" >> /var/tmp/crontest";
                system "echo \"gmcur_year, $gmcur_year\" >> /var/tmp/crontest";
                system "echo \"gmcur_wk, $gmcur_wk\" >> /var/tmp/crontest";
                system "echo \"gmcur_day, $gmcur_day\" >> /var/tmp/crontest";

                system "echo \"gmapt_day, $gmapt_day\" >> /var/tmp/crontest";
                system "echo \"gmapt_year, $gmapt_year\" >> /var/tmp/crontest";
                system "echo \"gmapt_mon, $gmapt_mon\" >> /var/tmp/crontest";
                system "echo \"gmapt_wk, $gmapt_wk\" >> /var/tmp/crontest";
                }
...
#
            ...
            if ($appttab{$onekey}{'recurtype'} eq "Daily") {
                ...
                if ($lg eq "manoj") {
                   system "echo \"recurtype DAILY ($onekey): $lg, $appthour:$apptmin $meridian, $apptday:$apptmonth:$apptyear\" >> /var/tmp/crontest";
                }
                ...
                if ($gmcur_year == $gmapt_year) {
                   if ($gmcur_mon == $gmapt_mon) {
                      if ($gmcur_day < $gmapt_day) {
                         if ($lg eq "manoj") {
                            system "echo \"gmcur_day($gmcur_day) < gmapt_day($gmapt_day) ($onekey): $lg, $appthour:$apptmin $meridian, $apptday:$apptmonth:$apptyear\" >> /var/tmp/crontest";
                         }
                         next;
                      }
                   } else {
...
# for our testing purposes only
                if ($lg eq "manoj") {
                   system "echo \"key = $onekey, mon = $mon, year = $year, day = $day, hour = $hour, min = $min \" >> /var/tmp/hdapptdue";
                }

# The day, month, and year in user's appt are some old dates. What we
# have done here is convert current time to gm time, and then we have
# generated user day, user mon, user year and used these as input to etimetosec
                $etime = etimetosec("", $apptmin, $appthour, $day, $mon, $year, "", "", "", $apptzone);

# this is for our testing purposes
                if ($lg eq "manoj") {
                   system "echo \"key = $onekey, etime = $etime\" >> /var/tmp/hdapptdue";
                }
                ...
                if ($lg eq "manoj")  {
                   system "echo \"etime:$etime\" >> /var/tmp/crontest";
                }
                #print "etime = $etime\n";
                if ($lg eq "manoj") {
                   system "echo \"DAILY APPT ($onekey): $lg, $appthour:$apptmin $meridian, $apptday:$apptmonth:$apptyear\" >> /var/tmp/crontest";
                }
            }
            ...
            if ($appttab{$onekey}{'recurtype'} eq "Weekly") {
              ...
              #print "etime = $etime\n";
              if ($lg eq "manoj") {
                 system "echo \"WEEKLY APPT ($onekey): $lg, $appthour:$apptmin $meridian, $apptday:$apptmonth:$apptyear\" >> /var/tmp/crontest";
              }
            }
...
#
            ...
            if (isaptcurrent($ctime, $etime)) { 
                  if ($lg eq "manoj") {
                     system "echo \"APT CURRENT ($onekey): $lg, $appthour:$apptmin $meridian, $apptday:$apptmonth:$apptyear\" >> /var/tmp/crontest";
                  }
                  ...
                  ($mfile = "/var/tmp/apptcron-$lg-$onekey-$$.txt") =~ s/\n/\n<BR>/g;
                  open mhandle, ">$mfile";
		  ...
		  print "Appt Event: Date: $apptmonth-$apptday-$apptyear \n";
                  ...
                  if (($counter % $ENV{'USE_APPT_WINNER_FREQ'}) == 0) { 
                     ...
                     system "cat $ENV{'HDHOME'}/letters/apptwinner > /var/tmp/apptwinner$onekey$$";
                     system "echo \"Entry Number  $onekey\" >> /var/tmp/apptwinner$onekey$$";
                     if ($email ne "") {
                        system "/bin/mail -s \"Did you just win a Prize!\" $email < /var/tmp/apptwinner$onekey$$";
                     }
                  }
    ...
    if ("\U$pagertype" eq "\UOther Pager") {
       ...
       $otherpagerfile = "/var/tmp/$login$otherpager$$";
   ...
   system "echo \"total logins processed = $mycnt\" >> /var/tmp/crontest";
