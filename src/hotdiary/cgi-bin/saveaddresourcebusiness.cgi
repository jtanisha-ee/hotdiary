...
#
# FileName: saveaddresourcebusiness.cgi
...
#session timeout in secs
   ...
   hddebug("saveaddresourcebusiness.cgi");
...
# bind login table vars
   ...
   system "mkdir -p $ENV{HDDATA}/business/business/$business/restab";
   system "chmod 755 $ENV{HDDATA}/business/business/$business/restab";
   system "chown nobody:nobody $ENV{HDDATA}/business/business/$business/restab";
   ...
   $resname = trim $input{resourcename};
   if ($resname eq "") {
      if ($os ne "nt") {
	 ...
	 $execmanageonebusiness = "execmanageonebusiness.cgi";
      ...
      status("$login: Resource Name is empty. Specify a name for the resource. Click <a href=\"http://$vdomain/cgi-bin/$rh/execdogeneric.cgi?p0=$execaddresourcebusiness&p1=biscuit&p2=business&pnum=4&p3=jp&re0=CGISUBDIR&le0=rh&re1=HTTPSUBDIR&le1=hs&re2=SERVER_NAME&le2=vdomain&re3=HDLIC&le3=HDLIC&HDLIC=$HDLIC&biscuit=$biscuit&jp=$jp&business=$business&rh=$rh&hs=$hs&vdomain=$vdomain&le4=os&re4=os&re5=HTTP_COOKIE&le5=HTTP_COOKIE&enum=6\">here</a> to add a resource. <BR>Click <a href=\"http://$vdomain/cgi-bin/$rh/execdogeneric.cgi?pnum=4&p0=$execmanageonebusiness&p1=biscuit&p2=business&p3=jp&re0=CGISUBDIR&le0=rh&re1=HTTPSUBDIR&le1=hs&re2=SERVER_NAME&le2=vdomain&re3=HDLIC&le3=HDLIC&HDLIC=$HDLIC&biscuit=$biscuit&jp=$jp&business=$business&rh=$rh&hs=$hs&vdomain=$vdomain&le4=os&re4=os&re5=HTTP_COOKIE&le5=HTTP_COOKIE&enum=6\">here</a> to manage business."); 
      exit;
   }

   if (notName($resname)) {
      status("Invalid characters in Name ($resname).  Click <a href=\"validation.html\"> here</a> for valid input.\n");
      exit;
   }

   if (notName($input{resourcetype})) {
      status("Invalid characters in Type ($input{resourcetype}).  Click <a href=\"validation.html\"> here</a> for valid input.\n");
      exit;
   }

   if (notName($input{resourcebldg})) {
      status("Invalid characters in Building ($input{resourcebldg}).  Click <a href=\"validation.html\"> here</a> for valid input.\n");
      exit;
   }
   if (notNumber($input{resourcezipcode})) {
      status("Invalid characters in Zipcode ($input{resourcezipcode}).  Click <a href=\"validation.html\"> here</a> for valid input.\n");
      exit;
   }
   if (notName($input{resourcecountry})) {
      status("Invalid characters in Country ($input{resourcecountry}).  Click <a href=\"validation.html\"> here</a> for valid input.\n");
      exit;
   }

   $restab{$resname}{'type'} = trim $input{'resourcetype'};
   $restab{$resname}{'bldg'} = trim $input{'resourcebldg'};
   $restab{$resname}{'zipcode'} = trim $input{'resourcezipcode'};
   $restab{$resname}{'country'} = trim $input{'resourcecountry'};
   $restab{$resname}{'tz'} = trim $input{'resourcetz'};
   $restab{$resname}{'name'} = $resname;
   
   #hddebug "type = $restab{$resname}{type}";
   #hddebug "bldg = $restab{$resname}{bldg}";
   #hddebug "zipcode = $restab{$resname}{zipcode}";
   #hddebug "country = $restab{$resname}{country}";
   #hddebug "tz = $restab{$resname}{tz}";
   #hddebug "name = $restab{$resname}{name}";
   ...
   system "/bin/cat $ENV{HDHREP}/$alphaindex/$login/manageresourcebusiness.html";
