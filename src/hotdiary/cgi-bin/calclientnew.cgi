...
# Accessing hidden fields in form (hdcalviewtop.html)
      ...
      if ($f eq "m") {
        if ($a eq "a") {
           addMeeting $etitle, $edtype, $edesc, $emonth, $eday, $eyear, $erecurtype, $ehour, $emin, $emeridian, $ezone, $eshare, $efree, $eatype, $edhour, $edmin, $login, $g, $econtact;
        }
        if ($a eq "e") {
           $en = $input{'en'}; 
           updateMeeting $en, $etitle, $edtype, $edesc, $emonth, $eday, $eyear, $erecurtype, $ehour, $emin, $emeridian, $ezone, $eshare, $efree, $eatype, $edhour, $edmin, $login, $g, $econtact;
        }
      }
   }
...
# Accessing hidden fields in form (hdcalviewtop.html)
   ...
   if (($a ne "a") && ($a ne "e")) {
      ...
      createCalendar $dy, $mo, $yr, "$ENV{HDHREP}/$repdir/cal$rand$biscuit.html", $login, $biscuit, $vw, $f, $a, $h, $m, $tz, $en, $sc, $jvw, $g, $publish, "$rh", "$logo", "$title", "$banner", "$vdomain", "$hs", "$jp", "$os";
