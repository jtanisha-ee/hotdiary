...
#
# FileName: sendrequesttojoin.cgi
...
# bind login table vars
   ...
   if (exists ($peopletab{$login})) {
      status("$login: You are already a member of ($business).<BR>Click <a href=\"http://$vdomain/cgi-bin/$rh/execdogeneric.cgi?pnum=2&p0=$execbusiness&p1=biscuit&re0=CGISUBDIR&le0=rh&re1=HTTPSUBDIR&le1=hs&re2=SERVER_NAME&le2=vdomain&re3=HDLIC&le3=HDLIC&HDLIC=$HDLIC&biscuit=$biscuit&rh=$rh&hs=$hs&vdomain=$vdomain&le4=os&re4=os&re5=HTTP_COOKIE&le5=HTTP_COOKIE&enum=6\">here</a> to return to Business home."); 
      exit;
   }
  
   if ($businesstab{$business}{businessmaster} eq $login) {
       status("$login: You are already a member of ($business) as you are Business Master for ($business).");
       exit; 
   } 
   
   # get the email address of businessmaster and send email requests to join 
   ...
   if (!(-d "$ENV{HDDATA}/business/business/$business/pendingtab")) {
      system "mkdir -p $ENV{HDDATA}/business/business/$business/pendingtab";
      system "chown nobody:nobody $ENV{HDDATA}/business/business/$business/pendingtab";
      system "chmod 755 $ENV{HDDATA}/business/business/$business/pendingtab";
   }
   
   # create an entry in the pendingta
   ...
   $pendingtab{$login}{business} = $business; 

   $emsg = "Dear $logtab{$businessmaster}{fname}, \n \n";
   $uname = $logtab{$login}{'fname'} . " " . $logtab{$login}{'lname'};
   $emsg .= "$uname has sent a request to join your ($business) Business. \n \n";
   $emsg .= "Business Name: ";
   $emsg .=  $business;
   $emsg .=  "\n\n";
   ...
   qx{/bin/mail -s \"Request From $uname\" $logtab{$businessmaster}{email} < /var/tmp/business$$};
