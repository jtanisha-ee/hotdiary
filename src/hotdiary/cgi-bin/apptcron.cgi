...
# bind cntrtab table vars
   ...
   $blocklist = qx{cat $ENV{HDDATA}/blocktab/blocktab.rec};
    ...
    foreach $fl (keys %remindtab) {
      ...
      #-e $tfile or next;
      if ( (!(-f "$ENV{HDDATA}/logtab/$fl.rec")) &&
           (-f "$ENV{HDDATA}/yplogtab/$fl.rec") ) {
         system "mv $ENV{HDDATA}/yplogtab/$fl.rec $ENV{HDDATA}/logtab";
      }
      if ( (!(-f "$ENV{HDDATA}/surveytab/$fl.rec")) &&
           (-f "$ENV{HDDATA}/ypsurveytab/$fl.rec") ) {
         system "mv $ENV{HDDATA}/ypsurveytab/$fl.rec $ENV{HDDATA}/surveytab";
      }

      $alph = substr $lg, 0, 1;
      $alph = $alph . '-index';
         (-e "$ENV{HDDATA}/$alph/$lg/appttab" and -d "$ENV{HDDATA}/$alph/$lg/appttab") or next;
      
      # bind personal appointment table vars
      if ($alph eq "a") {
      tie %appttab, 'AsciiDB::TagFile',
      DIRECTORY => "$ENV{HDDATA}/$alph/$lg/appttab",
      SUFIX => '.rec',
      SCHEMA => {
        ORDER => ['entryno', 'login', 'month', 'day', 'year',
        'hour', 'min', 'sec', 'meridian', 'dhour', 'dmin',
        'dtype', 'atype', 'desc', 'zone', 'recurtype',
        'share', 'free', 'subject', 'street', 'city', 'state', 'zipcode', 
	'country',  'venue', 'person', 'phone', 'banner', 'confirm', 
	'id', 'type'] };
      } else {
      tie %appttab, 'AsciiDB::TagFile',
      DIRECTORY => "$ENV{HDDATA}/$alph/$lg/appttab",
...
#
		  ...
		  print "Appt Event: Date: $apptmonth-$apptday-$apptyear \n";
                  ...
                  printf mhandle "\nHotDiary - New Generation Internet Organizerr, $hddomain80";
                  &flush(mhandle);
		    ...
		    #print "login =", $login, "\n";
		      ...
		      #print "entered login exists \n";
				...
				$dmin = "0";
                            ...
                            $prml = "";
                            $prml = strapp $prml, "template=$ENV{HDTMPL}/reminder.html";
                            $prml = strapp $prml, "templateout=$ENV{HDHOME}/tmp/reminder-$login$$.html";
                            ...
                            $prml = strapp $prml, "time=$apptmonth-$apptday-$apptyear $apthr:$apptmin $meridian";
                            ...
                            parseIt $prml;
			    system "/usr/bin/metasend -b -S 800000 -m \"text/html\" -f $ENV{HDHOME}/tmp/reminder-$login$$.html -s \"$login: $esubject\" -e \"\" -t \"$email\" -F cronuser\@$diary";
                         #}
			  ...
			  $pagertype = $logtab{$login}{'pagertype'};
                          ...
                          system "$ENV{HDEXECCGI}/execcleanproducts $pagerfile 1200";
    ...
    if ("\U$pagertype" eq "\UOther Pager") {
       ...
       system "$ENV{HDEXECCGI}/execcleanproducts $otherpagerfile 1200";
