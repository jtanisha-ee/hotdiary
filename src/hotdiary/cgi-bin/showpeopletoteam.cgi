...
# bind login table vars
   ...
   (@hshpeople) = sort keys %peopletab;
   if ($#hshpeople < 0) {
      status("$login: You do not have any members to invite to a team. Click >here</a> to invite members.");
      exit;
   }
   ...
   $msg .= "<TD><FONT FACE=\"Verdana\" SIZE=\"2\">Teams</FONT></TD>";
   $msg .= "</TR>";
   $smsg = $msg;
   $umsg = $msg;
   $cntr = 0;
   ...
   foreach $mem (sort keys %peopletab) {
       ...
       $teamlist = "";
       foreach $teamnm (sort keys %teamtab) {       
          if (-d ("$ENV{HDDATA}/business/business/$business/teams/$teamnm/teampeopletab")) { 
             tie %teampeopletab, 'AsciiDB::TagFile',
                DIRECTORY => "$ENV{HDDATA}/business/business/$business/teams/$teamnm/teampeopletab",
                SUFIX => '.rec',
                SCHEMA => {
                ORDER => ['login']};

             if (exists($teampeopletab{$mem})) {
		$teamlist .= $teamnm;
		$teamlist .= " ";
             } 
          } 
       ...
       if ($teamlist eq "") {
          $msg .= "<TD>&nbsp;</TD>";
           ...
           $msg .= "<TD><FONT FACE=\"Verdana\" SIZE=\"2\">$teamlist</FONT></TD>";
       }
   ...
   foreach $teamname (sort keys %teamtab) {
      if (exists ($teamtab{$teamname})) {
        $teams .= "<OPTION>$teamname";
       }
   }
   ...
   if ($os ne "nt") {
      ...
      $execaddpeopletoteam = encurl "execaddpeopletoteam.cgi";
      $execproxylogout = encurl "/proxy/execproxylogout.cgi";
      $execdeploypage =  encurl "execdeploypage.cgi";
      $execshowtopcal =  encurl "execshowtopcal.cgi";
   } else {
      $prb = strapp $prb, "formenc=";
      $execaddpeopletoteam = "execaddpeopletoteam.cgi";
      $execproxylogout = encurl "/proxy/execproxylogout.cgi";
      $execdeploypage =  encurl "execdeploypage.cgi";
      $execshowtopcal =  encurl "execshowtopcal.cgi";
   }
   ...
   $prb = strapp $prb, "template=$ENV{HDTMPL}/showpeopletoteam.html";
   $prb = strapp $prb, "templateout=$ENV{HDHREP}/$alphaindex/$login/showpeopletoteam-$$.html";
   ...
   $hiddenvars = "<INPUT TYPE=HIDDEN NAME=p0 VALUE=\"$execaddpeopletoteam\">";
   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p1 VALUE=biscuit>";
   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p2 VALUE=business>";
   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p3 VALUE=selbegin>";
   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p4 VALUE=selend>";
   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p5 VALUE=teamname>";
   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p6 VALUE=add>";
   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p7 VALUE=addall>";
   ...
   $statmsg = "Business Managers & Business Master can add members to a team.";
   ...
   #system "cat $ENV{HDHREP}/$alphaindex/$login/showpeopletoteam.html";
   hdsystemcat "$ENV{HDHREP}/$alphaindex/$login/showpeopletoteam-$$.html";
