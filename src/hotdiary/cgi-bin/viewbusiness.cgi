...
#session timeout in secs
   ...
   #print &PrintHeader;
   #print &HtmlTop ("viewbusiness.cgi example");
...
# bind login table vars
   ...
   foreach $business_idx (sort keys %businesstab) {
      if (-d "$ENV{HDDATA}/business/business/$business_idx/peopletab") { 
         ...
         if (($businesstab{$business_idx}{businessmaster} eq $login) ||
            ...
            if ( ($login eq "manoj") || ($login eq "buddie") || ($login eq "mjoshi") ) {
   ...
   $alph = $alph . '-index';
   $prml = strapp $prml, "templateout=$ENV{HDHREP}/$alph/$login/searchtoviewbusiness.html";
   ...
   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p0 VALUE=\"$execshowcontactlist\">";
   ...
   #system "cat $ENV{HDHREP}/$alph/$login/searchtoviewbusiness.html";
   hdsystemcat "$ENV{HDHREP}/$alph/$login/searchtoviewbusiness.html";
