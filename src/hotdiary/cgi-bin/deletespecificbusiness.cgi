...
#
# FileName: deletespecificbusiness.cgi
...
#session timeout in secs
   ...
   hddebug ("deletespecificbusiness.cgi");
...
# bind login table vars
   ...
   if (($login ne "") && ($business ne "") && ($ENV{HDDATA} ne "")) { 
      ...
      if (-d "$ENV{HDDATA}/business/business/people/$login/founded") {
         ...
         if (exists($founded{$business})) {
            delete $founded{$business};
	    tied(%founded)->sync();
         }
	    ...
	    for ($i = 0; $i <= $#hshpeople; $i = $i + 1) {
	       if (-d "$ENV{HDDATA}/business/business/people/$people/mybiztab"){
                  tie %mybiztab, 'AsciiDB::TagFile',
                  DIRECTORY => "$ENV{HDDATA}/business/business/people/$people/mybiztab",
                  SUFIX => '.rec',
                  SCHEMA => {
                  ORDER => ['business']};
	    ...
	    if (($business ne "") && ($login ne "") && ($ENV{HDDATA} ne "")) {
               system "rm -rf $ENV{HDDATA}/business/business/$business";
	    }
         }
      }    
   }
   ...
   #system "cat $ENV{HDHREP}/$login/business.html";
 

   tied(%sesstab)->sync();
   tied(%logsess)->sync();

   status("$login: Deleted business($business). <BR><LEFT><IMG SRC=\"http://www.hotdiary.com/images/check.gif\">Business calendars deleted. <BR><IMG SRC=\"http://www.hotdiary.com/images/check.gif\"> Business contacts from members, resellers, customers, and other folders are deleted. <BR><IMG SRC=\"http://www.hotdiary.com/images/check.gif\">Business resources and teams are deleted. <BR><IMG SRC=\"http://www.hotdiary.com/images/check.gif\">All other resources allocated for your business are deleted successfully.</LEFT>  <BR>Click <a href=\"http://$vdomain/cgi-bin/$rh/execdogeneric.cgi?pnum=3&p0=$execbusiness&p1=biscuit&p2=jp&re0=CGISUBDIR&le0=rh&re1=HTTPSUBDIR&le1=hs&re2=SERVER_NAME&le2=vdomain&re3=HDLIC&le3=HDLIC&HDLIC=$HDLIC&biscuit=$biscuit&jp=$jp&rh=$rh&hs=$hs&vdomain=$vdomain&le4=os&re4=os&re5=HTTP_COOKIE&le5=HTTP_COOKIE&enum=6\">here</a> to return to business home."); 
