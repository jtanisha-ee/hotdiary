...
#
# FileName: passwordsettingbusiness.cgi
# Purpose: Create A Virtual Business
...
#session timeout in secs
   ...
   hddebug("passwordsettingbusiness.cgi");
...
# bind login table vars
   ...
   $alphaindex = $alphaindex . '-index';
	...
	'businesstitle', 'street', 'suite', 'city', 'state', 
	'zipcode', 'country', 'phone', 'fax', 'other', 'list'] };                            
   ...
   tie %mgraccesstab, 'AsciiDB::TagFile', 
     ...
     SUFIX => '.rec',
     SCHEMA => {
     ORDER => ['access', 'pbusinessmaster', 'pbusinessmanager', 'pother', 'invite', 'approve', 'delete', 'edit', 'manage', 'contact', 'teams']};
   
   if (exists($mgraccesstab{access})) { 
      $pbusinessmaster = $mgraccesstab{access}{'pbusinessmaster'};  
      $pbusinessmanager = $mgraccesstab{access}{'pbusinessmanager'};  
      $pothers = $mgraccesstab{access}{'pother'};  
   } else {
      $pbusinessmaster = "";
      $pbusinessmanager = "";
      $pothers = "";
   ...
   if ($os ne "nt") {
      ...
      $execsavepasswordsettingbusiness = encurl "execsavepasswordsettingbusiness.cgi";
      ...
      $execsavepasswordsettingbusiness = "execsavepasswordsettingbusiness.cgi";
      $execproxylogout =  "/proxy/execproxylogout.cgi";
      $execdeploypage =  "execdeploypage.cgi";
      $execshowtopcal =  "execshowtopcal.cgi";
   }

   $prml = strapp $prml, "template=$ENV{HDTMPL}/passwordsettingbusiness.html";
   $prml = strapp $prml, "templateout=$ENV{HDHREP}/$alphaindex/$login/passwordsettingbusiness-$$.html";
   ...
   $hiddenvars = "<INPUT TYPE=HIDDEN NAME=pnum VALUE=6>";
   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p0 VALUE=\"$execsavepasswordsettingbusiness\">";
   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p1 VALUE=biscuit>";
   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p2 VALUE=business>";
   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p3 VALUE=pbusinessmaster>";
   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p4 VALUE=pbusinessmanager>";
   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p5 VALUE=pothers>";
   ...
   $prml = strapp $prml, "pbusinessmaster=$pbusinessmaster";
   $prml = strapp $prml, "pbusinessmanager=$pbusinessmanager";
   $prml = strapp $prml, "pothers=$pothers";
   ...
   #system "cat $ENV{HDHREP}/$alphaindex/$login/passwordsettingbusiness.html";
   hdsystemcat "$ENV{HDHREP}/$alphaindex/$login/passwordsettingbusiness-$$.html";
