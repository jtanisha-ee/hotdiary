...
#
# FileName: getcontactlist.cgi
# Purpose: get the contact list from the addressbook
...
# bind login table vars
   ...
   if ((!exists $businesstab{$business}) || ($business eq "")) {
      status("$login: Business ($business) does not exist. Click <a href=\"http://$vdomain/cgi-bin/$rh/execdogeneric.jsp?pnum=3&p0=$execcreatebusiness&p1=biscuit&p2=jp&re0=CGISUBDIR&le0=rh&re1=HTTPSUBDIR&le1=hs&re2=SERVER_NAME&le2=vdomain&re3=HDLIC&le3=HDLIC&HDLIC=$HDLIC&biscuit=$biscuit&jp=$jp&rh=$rh&hs=$hs&vdomain=$vdomain&le4=os&re4=os&re5=HTTP_COOKIE&le5=HTTP_COOKIE&enum=6\">here</a> to create business. <BR>Click <a href=\"http://$vdomain/cgi-bin/$rh/execdogeneric.jsp?pnum=3&p0=$execbusiness&p1=biscuit&p2=jp&re0=CGISUBDIR&le0=rh&re1=HTTPSUBDIR&le1=hs&re2=SERVER_NAME&le2=vdomain&re3=HDLIC&le3=HDLIC&HDLIC=$HDLIC&biscuit=$biscuit&jp=$jp&rh=$rh&hs=$hs&vdomain=$vdomain&le4=os&re4=os&re5=HTTP_COOKIE&le5=HTTP_COOKIE&enum=6\">here</a> to return to business home.");
      exit;
   }
       
   $selbegin = $input{numbegin};
   $selend = $input{numend};
   ...
   if ($action eq "Invite") { 
      for ($i = $selbegin; $i <= $selend; $i= $i + 1) {
         ...
         hddebug "checkboxval = $memberlogin";
         if ($checkboxval eq "on") {
            $email .= $logtab{$memberlogin}{email};
            $email .= " ";
         }
         $k = $k + 1;
      }
   ...
   hddebug "email = $email";
   system "echo \"$email\" >> $ENV{HDHOME}/tmp/$business-invitecontacts$$";
   ...
   tied(%logsess)->sync();
   status ("done");
   #system "/bin/cat $ENV{HDTMPL}/$alpha/$login/content.html";
   #system "/bin/cat $ENV{HDHREP}/$alpha/$login/hdbusinesscalmeetingview-$biscuit.html";
