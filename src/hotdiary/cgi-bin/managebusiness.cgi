...
#session timeout in secs
   ...
   hddebug("managebusiness.cgi example");
...
# bind login table vars
   ...
   foreach $business_idx (sort keys %businesstab) {
	  ...
	  if ( ($login eq "manoj") || ($login eq "mjoshi") || ($login eq "smitha") || ($login eq "buddie") ) {
	     $cntr = $cntr + 1;
   ...
   $prml = strapp $prml, "template=$ENV{HDTMPL}/managebusiness.html";
   $prml = strapp $prml, "templateout=$ENV{HDHREP}/$alphaindex/$login/managebusiness-$$.html";

   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p0 VALUE=\"$execmanageonebusiness\">";
   ...
   #system "cat $ENV{HDHREP}/$alphaindex/$login/managebusiness.html";
   hdsystemcat "$ENV{HDHREP}/$alphaindex/$login/managebusiness-$$.html";
