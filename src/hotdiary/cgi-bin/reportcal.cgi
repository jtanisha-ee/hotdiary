...
# bind login table vars
   ...
   if ($group eq "") {
       ...
       # bind personal appt table vars
        ...
        SCHEMA => {
           ORDER => ['entryno', 'login', 'month', 'day', 'year',
              ...
              'dtype', 'atype', 'desc', 'zone', 'recurtype', 'share', 'free',
             'subject'] };
        (-e "$ENV{HDDATA}/$alphao/$othermember/appttab" and -d "$ENV{HDDATA}/$alphao/$othermember/appttab") or return $prml;                      
        $lflag = 1;
      }
   } else {
    ...
    foreach $entryno (@entrynohsh) {
	  ...
	  next;
       ...
       $msg = "<TR>";
       #$msg .= "<TD></TD>";
       $msg .= "<TD ALIGN=CENTER><FONT FACE=\"Verdana\" SIZE=\"2\">$monstr $appttab{$entryno}{day}</FONT></TD>";
   ...
   if ($rh eq "") {
      ...
      #$listall = adjusturl "/cgi-bin/$rh/execlistappts.cgi?biscuit=$biscuit&jp=$jp";
      $listall = adjusturl "/cgi-bin/execlistappts.cgi?biscuit=$biscuit&jp=$jp";
   }
   

   $prml = strapp $prml, "percal=$percal";
   $prml = strapp $prml, "listall=$listall";
