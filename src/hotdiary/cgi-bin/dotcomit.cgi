...
# bind master table vars
   ...
   $mastertab{$email}{contacted} = "self";
   $mastertab{$email}{yearsinbusiness} = $input{yearsinbusiness};
   $mastertab{$email}{held} = $input{held};
   $mastertab{$email}{numemp} = $input{numemp};
   $mastertab{$email}{annualrev} = $input{annualrev};
   $mastertab{$email}{symbol} = $input{symbol};
   $mastertab{$email}{description} = $input{description};
   $mastertab{$email}{zipcode} = $input{zipcode};
   ...
   status("$login: Your company entry <p><b>$url</b><p>, email<p><b>$email</b><p> and other information has been added to HotDiary Internet directory. HotDiary will be announcing the complete directory with your potential selection on May 15th, 2000. Please check back with us, to see if your name has been selected. Click <a href=\"http://www.hotdiary.com/dotcomit.html\"> to add</a> another company.");
   exit;
