...
#session timeout in secs
   ...
   #print &PrintHeader;
   #print &HtmlTop ("editmergedcal.cgi example");
...
# bind lmergetabb table vars
   ...
   if ("\L$lmergetab{$g}{'password'}" eq "\L$password") {
      # this is redirect url, will be invoked when Add button is pressed
      if ($rh eq "") {
         $cgis = adjusturl "/cgi-bin/execmgcalclient.cgi?biscuit=$biscuit&g=$g&jp=$jp";
      } else {
         $cgis = adjusturl "/cgi-bin/$rh/execmgcalclient.cgi?biscuit=$biscuit&g=$g&jp=$jp";
      }
      $prm = "";
