...
# bind login table vars
   ...
   $msg .= "<TD WIDTH=\"10%\"><FONT FACE=\"Verdana\" SIZE=\"2\" COLOR=ffffff>Time(Zone) &nbsp;</FONT></TD>";
   ...
   $msg .= "<TD WIDTH=\"10%\"><FONT FACE=\"Verdana\" SIZE=\"2\" COLOR=ffffff>Teams</FONT></TD>";
   $msg .= "<TD WIDTH=\"10%\"><FONT FACE=\"Verdana\" SIZE=\"2\" COLOR=ffffff>Groups</FONT></TD>";
   ...
   foreach $entryno (sort keys %appttab) {
     if ($appttab{$entryno}{type} eq "meeting") {
       if ($appttab{$entryno}{confirm} eq "no") {

         ($business, $rem) = split(" ", $appttab{$entryno}{id});
         if ($business eq "") {
  	    $business = calfuncs::bizfuncs::getbiz($entryno, $login);
	 }
	 if ($business eq "") {
	    next;
	 }
	 if (!-d "$ENV{HDDATA}/business/business/$business/meetingtab") {
	   ...
	   $yes = "";
           ...
           $msg .= "<TD><INPUT TYPE=CHECKBOX NAME=$entryno></TD>";
	   $cdir .= "$entryno ";
	   ...
	   if ($appttab{$entryno}{subject} eq "") {
	      ...
	      $subject = $appttab{$entryno}{subject};
           ...
           $msg .= "<TD WIDTH=\"10%\"><FONT FACE=Verdana SIZE=2>$subject &nbsp;</FONT></TD>";
           ...
           for ($i = 0; $i <= $#hshmems; $i = $i + 1) {
              tie %appttab, 'AsciiDB::TagFile',
              DIRECTORY => "$ENV{HDDATA}/$hshmems[$i]/appttab",
              SUFIX => '.rec',
              SCHEMA => {
                   ORDER => ['entryno', 'login', 'month', 'day', 'year',
                   'hour', 'min', 'meridian', 'dhour', 'dmin',
              ...
              if (exists($appttab{$entryno} )) {
                 if ($appttab{$entryno}{confirm} eq "no") {
                    $no .= "$logtab{$hshmems[$i]}{fname}$logtab{$hshmems[$i]}{lname}($hshmems[$i])";
                 } else {
	   ...
	   ($nno, $yyes, $num_mem) = calfuncs::bizfuncs::getmeminfo($meetingtab{$entryno}{mem}, $entryno);
	   hddebug "$num_mem, yes = $yyes, no = $nno";
	   $no .= $nno;
	   $yes .= $yyes;
	   hddebug "$num_mem, yes = $yes, no = $no";
	   $numinvitees = $meetingtab{$entryno}{numinvitees} + $num_mem;
	   ...
	   SCHEMA => {
           ...
           $msg .= "<TD WIDTH=\"10%\"><FONT FACE=Verdana SIZE=2>$numinvitees &nbsp;</FONT></TD>";
           ...
           $msg .= "<TD WIDTH=\"10%\"><FONT FACE=Verdana SIZE=2>$meetingtab{$entryno}{teams} &nbsp;</FONT></TD>";
           $msg .= "<TD WIDTH=\"10%\"><FONT FACE=Verdana SIZE=2>$meetingtab{$entryno}{groups} &nbsp;</FONT></TD>";
         }
         $msg .= "</TR>";
         $smsg .= $msg;
      }
   ...
   if ($os ne "nt") {
      ...
      $execconfirmrsvp =  encurl "execconfirmrsvp.cgi";
   } else {
      ...
      $execconfirmrsvp =  "execconfirmrsvp.cgi";
   }

   $prb = strapp $prb, "template=$ENV{HDTMPL}/personalrsvp.html";
   $prb = strapp $prb, "templateout=$ENV{HDHREP}/$alphaindex/$login/personalrsvp-$$.html";
   ...
   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p0 VALUE=$execconfirmrsvp>";
   ...
   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p4 VALUE=business>";
   ...
   #system "cat $ENV{HDHREP}/$alphaindex/$login/personalrsvp.html";
   hdsystemcat "$ENV{HDHREP}/$alphaindex/$login/personalrsvp-$$.html";
