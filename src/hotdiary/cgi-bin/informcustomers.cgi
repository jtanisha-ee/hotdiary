...
$mfile = "$ENV{HDHOME}/letters/newsletter-Mar2003.html";
$subject = "Redbasin Networks Newsletter - Mar. 2003";
...
foreach $account (sort keys %logtab) {
  ...
  if ($logtab{$account}{'informme'} eq "CHECKED") {
     ...
     $password = $logtab{$account}{password};
     if ($email eq "") {
        next;
     }
     ...
     $cntr += 1;
     print "Sending newsletter-Mar-$$-$account.html to $email...";
     qx{sleep 1};
     system "echo \"$email\" >> $mfile.out"; 
     #system "/bin/mail -s \"$subject\" \"$email\" < $mfile";
     $prml = "";
     $prml = strapp $prml, "template=$mfile";
     $prml = strapp $prml, "templateout=$ENV{HDHOME}/tmp/newsletter-Mar-$$-$account.html";
     ...
     parseIt $prml;     
     system "metasend -b -S 800000 -m \"text/html\" -f $ENV{HDHOME}/tmp/newsletter-Mar-$$-$account.html -s \"$subject\" -e \"\" -t \"$email\" -F noreply\@hotdiary.com";
     #system "metasend -b -S 800000 -m \"text/html\" -f $mfile -s \"$subject\" -e \"\" -t \"$email\" -F noreply\@hotdiary.com";
     #system "rm -f $ENV{HDHOME}/tmp/newsletter-Mar-$$-$account.html";
     print "...Done.\n";
  } 
}
