...
#
               ...
               if (isaptcurrent($ctime, $etime)) { 
                  ...
                  ($mfile = "$ENV{HDHOME}/tmp/companycalcron$$.txt") =~ s/\n/\n<BR>/g;
		  ...
		  foreach $t (keys %peopletab) {
		     $uname = trim $t;
		     $login = $uname;
                     if (!exists $logtab{$uname}) {
	                next;
                     }
                     $email = $logtab{$uname}{'email'};

		     print "-----Employee Name: $uname \n";
                     print "-----Entry Number: $onekey \n";
		     print "-----Company Calendar Reminder Event: Date: $apptmonth-$apptday-$apptyear \n";
                     print "-----Company Calendar Reminder Event: Time: $apthr:$apptmin $meridian\n";
		     ...
		     $dmin = "0";
                  ...
                  if ($atype eq "Email") {
                    if (exists $logtab{$login}) {
                      if ($email ne "") {
                         system "/usr/bin/metasend -b -S 800000 -m \"text/html\" -f $ENV{HDHOME}/tmp/reminder-$login$$.html -s \"$esubject\" -e \"\" -t \"$email\" -F cronuser\@hotdiary.com";
                      }
                    } #else
                  } else {
                    if ($atype eq "Pager") {
