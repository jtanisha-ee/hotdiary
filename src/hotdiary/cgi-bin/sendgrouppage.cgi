...
# bind login table vars
   ...
   $successpage = "http://$vdomain/cgi-bin/$rh/execdogeneric.cgi?pnum=4&p0=$execfile&p1=biscuit&p2=jp&p3=g&re0=CGISUBDIR&le0=rh&re1=HTTPSUBDIR&le1=hs&re2=SERVER_NAME&le2=vdomain&re3=HDLIC&le3=HDLIC&HDLIC=$HDLIC&biscuit=$biscuit&jp=$jp&g=$g&rh=$rh&hs=$hs&vdomain=$vdomain&le4=os&re4=os&re5=HTTP_COOKIE&le5=HTTP_COOKIE&enum=6";

   $successpage = goodwebstr($successpage);

   qx{echo \"$message\" > /var/tmp/$g$$};
   #if ($action = "Send") { 
       if ("\U$pagertype" eq "\USkyTel Pager") {
	  hddebug "came here";
          $prml = "";
          ...
          $prml = strapp $prml, "templateout=$ENV{HDHOME}/tmp/$g-redirect_url-$$.html";
          $prml = strapp $prml, "redirecturl=$redirecturl";
          parseIt $prml;         
          system "cat $ENV{HDTMPL}/content.html"; 
          system "cat $ENV{HDHOME}/tmp/$g-redirect_url-$$.html"; 
          exit;
       } else  {
	  if ("\U$pagertype" eq "\UAirTouch Pager") {
	       $email = "$to\@airtouch.net";
               system "/bin/mail -s \"$from\" $email < /var/tmp/$g$$";	
               #system "cat $ENV{HDTMPL}/content.html"; 
               #$successpage = qx{basename $successpage};
               #system "cat $ENV{HDREP}/$g/$successpage"; 
	  } else  {
	     if ("\U$pagertype" eq "\UNextel Pager") {
	         $email = "$to\@page.nextel.com";
                 system "mail -s \"$from\" $email < /var/tmp/$g$$";	
                 #system "cat $ENV{HDTMPL}/content.html"; 
                 #$successpage = qx{basename $successpage};
                 #system "cat $ENV{HDREP}/$g/$successpage"; 
             } else {
	            ...
	            $email = "$to\@pagemart.net";
	             ...
	             if ("\U$pagertype" eq "\UMetrocall Pager") {
                        ...
                        #system "cat $ENV{HDREP}/$g/$successpage"; 
                          ...
                          system "mail -s \"$from\" $to < /var/tmp/$g$$";	
                          #system "cat $ENV{HDTMPL}/content.html"; 
                          #$successpage = qx{basename $successpage};
                          #system "cat $ENV{HDREP}/$g/$successpage"; 
                          #status("$g: You have selected \"Other Pager\", which is not supported for communication from HotDiary.");
                          #exit;
                     }
                 }
             }
...
# save the info in db
   ...
   system "cat $ENV{HDTMPL}/content.html";
   system "cat $ENV{HDHOME}/tmp/$g-$dirtype.html";
