...
# bind login table vars
  ...
  SCHEMA => {
   ...
   if ($os ne "nt") {
      ...
      $execsendgrouppage = encurl "execsendgrouppage.cgi";
      $execproxylogout = encurl "/proxy/execproxylogout.cgi";
      $execdeploypage =  encurl "execdeploypage.cgi";
      $execshowtopcal =  encurl "execshowtopcal.cgi";
   } else {
      $prml = strapp $prml, "formenc=";
      $execsendgrouppage = "execsendgrouppage.cgi";
   ...
   if ( ($jp ne "") && 
      (-f "$ENV{HDDATA}/$alphj/$jp/templates/showgrouppage.html")) {
      $template = "$ENV{HDDATA}/$alphj/$jp/templates/showgrouppage.html";
   } else {
      $template = "$ENV{HDTMPL}/showgrouppage.html";
   }    
   ...
   if ($biscuit eq "") {
      $prml = strapp $prml, "templateout=$ENV{HDHOME}/tmp/$g-showgrouppage$$.html";
   } else {
      $prml = strapp $prml, "templateout=$ENV{HDHREP}/$alphaindex/$login/showgrouppage-$$.html";
   }
   ...
   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p0 VALUE=\"$execsendgrouppage\">";
   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p1 VALUE=biscuit>";
   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p2 VALUE=to>";
   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p3 VALUE=thispage>";
   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p4 VALUE=pagertype>";
   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p5 VALUE=message>";
   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p6 VALUE=dirtype>";
   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p7 VALUE=jp>";
   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p8 VALUE=g>";
   ...
   if ($biscuit eq "") {
      #system "/bin/cat $ENV{HDHOME}/tmp/$g-showgrouppage$$.html"; 
      hdsystemcat "$ENV{HDHOME}/tmp/$g-showgrouppage$$.html"; 
   } else {
      #system "/bin/cat $ENV{HDHREP}/$alphaindex/$login/showgrouppage.html"; 
      hdsystemcat "$ENV{HDHREP}/$alphaindex/$login/showgrouppage-$$.html"; 
      tied(%sesstab)->sync();
      tied(%logsess)->sync();
   }
}
