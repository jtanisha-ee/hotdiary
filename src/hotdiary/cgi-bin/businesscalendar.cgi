...
#
# FileName: businesscalendar.cgi
# Purpose: Business Calendar 
...
# bind login table vars
   ...
   (@hshnumbus) = sort keys %businesstab;
   ...
   foreach $business_idx (sort keys %businesstab) {
      ...
      if (-d "$ENV{HDDATA}/business/business/$business_idx/peopletab") { 
	 ...
	 if (($businesstab{$business_idx}{businessmaster} eq $login) ||
	    ...
	    if ( ($login eq "smitha") || ($login eq "mjoshi") || ($login eq "buddie") ) {
              $sel .= "\<OPTION\>$businesstab{$business_idx}{business}\<\/OPTION\>";
	      $cntr = $cntr + 1;
	    } else {
   ...
   $prml = "";
   $prml = strapp $prml, "businesslist=$sel";
   if ($logo ne "") {
         $logo = adjusturl $logo;
   }
   $rh = $input{rh};
   ...
   $alpha = $alpha . '-index';
   $prml = strapp $prml, "template=$ENV{HDTMPL}/searchtoviewbusiness.html";
   $prml = strapp $prml, "templateout=$ENV{HDHREP}/$alpha/$login/searchtoviewbusiness-$$.html";
   ...
   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p0 VALUE=\"$execshowbusinesscalmenu\">";
   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p1 VALUE=biscuit>";
   $hiddenvars .= "<INPUT TYPE=HIDDEN NAME=p2 VALUE=businesslist>";
   ...
   #system "cat $ENV{HDHREP}/$login/searchtoviewbusiness.html";
   hdsystemcat "$ENV{HDHREP}/$alpha/$login/searchtoviewbusiness-$$.html";
