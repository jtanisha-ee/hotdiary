...
# parse the command line
   ...
   hddebug "$member Reading ShockTheWave Newsletter May-2000: $referer";
   #if ($member ne "") {
   ...
   if (exists $logtab{$member}) {
      ...
      if ( ! -d "$ENV{HDDATA}/$alphaindex/$member/uniquetab" ) {
         system "mkdir -p $ENV{HDDATA}/$alphaindex/$member/uniquetab";
      }
   
      $moneytab{$member}{'login'} = $member;
   
      tie %uniquetab, 'AsciiDB::TagFile',
          DIRECTORY => "$ENV{HDDATA}/$alphaindex/$member/uniquetab",
          SUFIX => '.rec',
          SCHEMA => {
