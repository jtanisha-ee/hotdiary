...
# bind login table vars
   ...
   if ($login eq "") {
      if ($hs eq "") {
         status("Please enter a non-empty login string. Click <a href=\"http://$vdomain/index.html\" TARGET=\"_parent\"> here</a> to login.");
      } else {
         status("Please enter a non-empty login string. Click <a href=\"http://$vdomain/$hs/index.html\" TARGET=\"_parent\"> here</a> to login.");
      }
      exit;
   }
...
##### CAE END
          ...
          if ($hs eq "") {
              status("$login: Could not login. Enter correct password.  Click <a href=\"http://$vdomain/index.html\" TARGET=\"_parent\"> here</a> to login. If you have forgotten your password, click <a href=\"forgotpasswd.html\">here.</a>");
          } else {
              status("$login: Could not login. Enter correct password.  Click <a href=\"http://$vdomain/$hs/index.html\" TARGET=\"_parent\"> here</a> to login."); 
          }
          exit;
          }
       }
...
# if we reached here, the login was successful, display the choice screen
   ...
   $prml = "";
   $prml = strapp $prml, "templateout=$ENV{HDHREP}/$login/red-$biscuit-$$.html";
   $prml = strapp $prml, "template=$ENV{HDTMPL}/proxy/redirect.html";
   $rh = $input{'rh'};
   $url = adjusturl "/cgi-bin/$rh/execdocalclient.cgi?biscuit=$biscuit&vw=m";
   $prml = strapp $prml, "redirecturl=$url";
   parseIt $prml;

   system "cat $ENV{HDTMPL}/content.html";
   system "cat $ENV{HDHREP}/$login/red-$biscuit-$$.html";
